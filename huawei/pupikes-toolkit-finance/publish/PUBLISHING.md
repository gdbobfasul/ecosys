# Документи за публикуване — Pupikes Toolkit Finance
...
- **RuStore пакет:** `com.pupikes.toolkitfinance.rustore`
- **Билд (APK/AAB):** идва от `apk/pupikes-toolkit-finance-{huawei,rustore}-debug.apk` след меню 57 (не е в тази папка).
...
> Пакетът вече е `com.pupikes.toolkitfinance.hw` (сменен от стария `com.kcy.toolkitfinance.hw`). AppGallery **не позволява** смяна на
> пакета на съществуващ запис — ако качиш в стария, отказва с „_the name of the uploaded package is
> different from the existing package name com.kcy.toolkitfinance.hw_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitfinance.hw`** (фиксира се веднъж — трябва да е точно това).
> 2. Попълни данните по таблицата долу и качи APK-то в **новия** запис.
> 3. Ако старият `com.kcy.toolkitfinance.hw` е бил публикуван — свали го от продажба СЛЕД одобрение на новия.
...
> Пакетът вече е `com.pupikes.toolkitfinance.rustore` (сменен от стария `com.kcy.toolkitfinance.rustore`). RuStore, както и Huawei, **не позволява**
> смяна на пакета на съществуващ запис. Затова: създай **ново приложение** с
> **applicationId = `com.pupikes.toolkitfinance.rustore`** и качи APK-то там. Ако старият `com.kcy.toolkitfinance.rustore` е бил публикуван — свали
...
| **Политика за поверителност (руски)** | подава се като URL в RuStore Console + в апа | [`rustore-privacy.html`](./rustore-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/rustore-privacy.html` |
...
_Политики онлайн: Huawei → `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/hw-privacy.html` · RuStore → `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/rustore-privacy.html` (качват се на сървъра чрез меню 08 — sync_privacy_pages)._
