...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/duel-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/duel/hw-privacy.html` |
...
```
Launch the app; on first run choose a language and tap Start, then press Start/Begin in the menu. Fight turn by turn: pick a basic attack, or try key combinations to discover the hero's hidden special move. No account, login, or internet is required; it runs entirely offline and only asks for an optional leaderboard name. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
