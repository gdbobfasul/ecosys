...
// (core/pupikes-bar.js) + showcase списък на ДРУГИТЕ приложения (снимка + ИМЕ + описание 15 езика +
...
//
// Каталогът се тегли ПЪРВО ОТ СЪРВЪРА (public/promo/pupikes-promo.json → https://…/promo/pupikes-promo.json):
...
// `pupikes-promo.json`, вкарано в билда от `app-shared/promo-catalog.json`.
...
export function mountEcosystem(selfId) {
  try {
    if (!document.getElementById('pupikes-paw-halo-style')) {
      const st = document.createElement('style'); st.id = 'pupikes-paw-halo-style';
      st.textContent = '#pupikes-eco-btn{display:inline-flex;align-items:center;justify-content:center;padding:5px 7px;border-radius:50%;animation:pupikesPawHalo 1.8s ease-in-out infinite}#pupikes-eco-btn svg{display:block}' +
        '@keyframes pupikesPawHalo{0%,100%{box-shadow:0 0 5px 1px rgba(130,195,255,.35)}50%{box-shadow:0 0 13px 5px rgba(130,195,255,.75)}}';
      (document.head || document.documentElement).appendChild(st);
    }
  } catch (e) {}
  const self = selfId || '';
  function openShowcase() {
    if (document.getElementById('pupikes-eco-ov')) return;
    const ov = document.createElement('div'); ov.id = 'pupikes-eco-ov';
    ...
    ov.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid #1b2536">' +
        ...
        '<button id="pupikes-eco-x" style="background:#1b2536;color:#cdd;border:none;border-radius:8px;padding:8px 12px;font-size:16px;cursor:pointer">✕</button></div>' +
      '<div id="pupikes-eco-list" style="flex:1;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:12px"><div style="text-align:center;opacity:.55;padding:30px">…</div></div>' +
    ...
    document.getElementById('pupikes-eco-x').onclick = () => ov.remove();
    loadCatalog().then((cat) => {
      const list = document.getElementById('pupikes-eco-list'); if (!list) return;
  ...
  pupikesBarButton({ id: 'pupikes-eco-btn', order: 10, accent: true, html: true, label: () => PUPIKES_PAW, onClick: openShowcase });
}
