...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/rustam-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/rustam/hw-privacy.html` |
...
```
No account, login or internet connection is needed. Launch the app, choose a language on first run, and tap Start to play. Collect the cucumbers before the moles reach them and advance through the 10 levels. Everything runs offline on the device. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
