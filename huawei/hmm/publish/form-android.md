...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/hmm-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/hmm/hw-privacy.html` |
...
```
Launch the app; on first run choose a language, then start the battle from the menu (Start / Space). Command your 3 heroes turn by turn: pick basic attacks or try key combinations to unlock each hero's hidden special. No account, login, or internet is required; it runs entirely offline and only asks for an optional leaderboard name. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
