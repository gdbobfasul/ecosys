# Документи за публикуване — Pupikes Field Battle
...
> Пакетът вече е `com.pupikes.hmm.hw` (сменен от стария `com.kcy.hmm.hw`). AppGallery **не позволява** смяна на
> пакета на съществуващ запис — ако качиш в стария, отказва с „_the name of the uploaded package is
> different from the existing package name com.kcy.hmm.hw_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.hmm.hw`** (фиксира се веднъж — трябва да е точно това).
> 2. Попълни данните по таблицата долу и качи APK-то в **новия** запис.
> 3. Ако старият `com.kcy.hmm.hw` е бил публикуван — свали го от продажба СЛЕД одобрение на новия.
...
> Пакетът вече е `com.pupikes.hmm.rustore` (сменен от стария `com.kcy.hmm.rustore`). RuStore, както и Huawei, **не позволява**
> смяна на пакета на съществуващ запис. Затова: създай **ново приложение** с
> **applicationId = `com.pupikes.hmm.rustore`** и качи APK-то там. Ако старият `com.kcy.hmm.rustore` е бил публикуван — свали
...
_Политики онлайн: Huawei → `https://selflearning.bot.nu/privacy/hmm/hw-privacy.html` · RuStore → `https://selflearning.bot.nu/privacy/hmm/rustore-privacy.html` (качват се на сървъра чрез меню 08 — sync_privacy_pages)._
