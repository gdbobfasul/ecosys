# Android форма — aip1-moodpatterns
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-moodpatterns-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-moodpatterns/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Add an entry by choosing how you feel, optionally tag what happened and write a note, then Save; the mood chart, statistics and pattern/tag insights are computed on the device and work offline. To try the optional AI-assisted insights, open the gear/settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device), then press the weekly or monthly insight button; only then are the chosen entries sent to that provider. History is stored locally and can be exported or cleared. This is a journaling and self-reflection tool, not medical or psychological advice. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-moodpatterns-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1moodpatterns.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-moodpatterns/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
