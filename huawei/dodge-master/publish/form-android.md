...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/dodge-master-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/dodge-master/hw-privacy.html` |
...
```
Launch the app; on first run pick a language and tap Start, then choose a level and press PLAY. Move your character to dodge incoming projectiles until the level ends. No account, login, or internet connection is required; an optional name for the local leaderboard is the only input. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
