...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-guardian-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-guardian/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. The inner app interface is in Bulgarian. Type a short note, choose a situation template and add context, then request the risk read — this works on-device with built-in logic and needs no key. To try the optional AI-assisted mode, open settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the context is sent to that provider only when you request an analysis. The silent help button and the alerts to trusted contacts are SIMULATED in this prototype — nothing is actually dispatched; it is not a real emergency service. All data is stored locally and can be cleared. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-guardian-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1guardian.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-guardian/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
