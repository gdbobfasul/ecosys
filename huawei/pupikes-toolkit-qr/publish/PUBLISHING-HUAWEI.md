# Публикуване в Huawei AppGallery — Pupikes Toolkit QR
...
- **Huawei пакет:** `com.pupikes.toolkitqr.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-QR-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkitqr.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitqr.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-qr/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit QR |
| bg | Български | **Bulgarian** | Pupikes Toolkit QR |
| ru | Русский | **Russian** | Pupikes Toolkit QR |
| uk | Українська | **Ukrainian** | Pupikes Toolkit QR |
| de | Deutsch | **German** | Pupikes Toolkit QR |
| fr | Français | **French (France)** | Pupikes Toolkit QR |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit QR |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit QR |
| it | Italiano | **Italian** | Pupikes Toolkit QR |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit QR |
| ar | العربية | **Arabic** | Pupikes Toolkit QR |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit QR |
| ja | 日本語 | **Japanese** | Pupikes Toolkit QR |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit QR |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-qr/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-QR-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-qr/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
