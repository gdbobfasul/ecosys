# Android форма — aip1-symptombridge
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-symptombridge-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-symptombridge/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Answer the guided questions or type your complaint freely, then press Build to get a structured, doctor-ready summary; this works fully offline. You can keep several profiles (e.g. family members), save a visit to the local history, and copy or print the summary. The optional AI mode is off by default: open the gear/settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the symptom text is sent to that provider only when you press the AI action. The app does not diagnose and is not a medical device. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-symptombridge-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1symptombridge.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-symptombridge/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
