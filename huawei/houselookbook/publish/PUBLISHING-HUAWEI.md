# Публикуване в Huawei AppGallery — HouseLookBook
...
- **Билд (APK/AAB):** `apk/huawei/release/HouseLookBook-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.houselookbook.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.houselookbook.hw`** (фиксира се веднъж — точно това).
...
| Екранни снимки | поне 3; споделени + по език (4 папки/файла) | [`screenshots/`](./screenshots/) |
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/houselookbook/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „Lifestyle" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | HouseLookBook |
| bg | Български | **Bulgarian** | HouseLookBook |
| ru | Русский | **Russian** | HouseLookBook |
| uk | Українська | **Ukrainian** | HouseLookBook |
| de | Deutsch | **German** | HouseLookBook |
| fr | Français | **French (France)** | HouseLookBook |
| es | Español | **Spanish (Spain)** | HouseLookBook |
| es-MX | Español (MX) | **Spanish (Latin America)** | HouseLookBook |
| it | Italiano | **Italian** | HouseLookBook |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | HouseLookBook |
| ar | العربية | **Arabic** | HouseLookBook |
| hi | हिन्दी | **Hindi** | HouseLookBook |
| ja | 日本語 | **Japanese** | HouseLookBook |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | HouseLookBook |
...
| **Screenshots** (задължително) | екраните от `screenshots/` (поне 3) — налични 4. Препоръка: избор на език, интро, съгласие, начален екран |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/houselookbook/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
- Събиране на лични данни: **да** · Акаунт/вход: **да** → дай тестов акаунт.

### 7. New version (билд)
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/HouseLookBook-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/houselookbook/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
