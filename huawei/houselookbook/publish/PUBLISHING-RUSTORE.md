# Публикуване в RuStore — HouseLookBook
...
- **Билд (APK/AAB):** `apk/rustore/release/HouseLookBook-rustore-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.houselookbook.rustore`. RuStore, както Huawei, **не позволява** смяна на пакета на съществуващ запис.
> Създай **ново приложение** с **applicationId = `com.pupikes.houselookbook.rustore`** и качи APK-то там. Ако е съществувал стар
...
| Поле | Стойност |
|---|---|
| applicationId | `com.pupikes.houselookbook.rustore` |
| Название приложения | `HouseLookBook` |
...
| Скриншоты | `screenshots/` (1–10) — налични 4 |

### 4. Разрешения и данни
- **Политика конфиденциальности (URL):** `https://selflearning.bot.nu/privacy/houselookbook/rustore-privacy.html` (адрес; същата се отваря и в апа).
...
- Сбор персональных данных: **да** · Аккаунт/вход: **да**.
...
| Поле | Стойност |
|---|---|
| APK/AAB | качи `apk/rustore/release/HouseLookBook-rustore-release.apk` |

### 7. Преди подаване
Прегледай `analyse.rustore` → **Отправить на модерацию**.

_Политика онлайн (RuStore): `https://selflearning.bot.nu/privacy/houselookbook/rustore-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
