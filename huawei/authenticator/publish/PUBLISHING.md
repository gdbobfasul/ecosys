# Документи за публикуване — Pupikes Toolkit Authenticator
...
- **RuStore пакет:** `com.pupikes.authenticator.rustore`
- **Билд (APK/AAB):** идва от `apk/authenticator-{huawei,rustore}-debug.apk` след меню 57 (не е в тази папка).
...
> Пакетът вече е `com.pupikes.authenticator.hw` (сменен от стария `com.kcy.authenticator.hw`). AppGallery **не позволява** смяна на
> пакета на съществуващ запис — ако качиш в стария, отказва с „_the name of the uploaded package is
> different from the existing package name com.kcy.authenticator.hw_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.authenticator.hw`** (фиксира се веднъж — трябва да е точно това).
> 2. Попълни данните по таблицата долу и качи APK-то в **новия** запис.
> 3. Ако старият `com.kcy.authenticator.hw` е бил публикуван — свали го от продажба СЛЕД одобрение на новия.
...
> Пакетът вече е `com.pupikes.authenticator.rustore` (сменен от стария `com.kcy.authenticator.rustore`). RuStore, както и Huawei, **не позволява**
> смяна на пакета на съществуващ запис. Затова: създай **ново приложение** с
> **applicationId = `com.pupikes.authenticator.rustore`** и качи APK-то там. Ако старият `com.kcy.authenticator.rustore` е бил публикуван — свали
...
| **Политика за поверителност (руски)** | подава се като URL в RuStore Console + в апа | [`rustore-privacy.html`](./rustore-privacy.html) → `https://selflearning.bot.nu/privacy/authenticator/rustore-privacy.html` |
...
_Политики онлайн: Huawei → `https://selflearning.bot.nu/privacy/authenticator/hw-privacy.html` · RuStore → `https://selflearning.bot.nu/privacy/authenticator/rustore-privacy.html` (качват се на сървъра чрез меню 08 — sync_privacy_pages)._
