...
register({
  ...
  pw_last: { bg:'Последно сканиране: {0}', ru:'Последнее сканирование: {0}', uk:'Останнє сканування: {0}', en:'Last scan: {0}', de:'Letzter Scan: {0}', fr:'Dernier scan : {0}', es:'Último escaneo: {0}', 'es-MX':'Último escaneo: {0}', it:'Ultima scansione: {0}', pt:'Última análise: {0}', ar:'آخر فحص: {0}', hi:'अंतिम स्कैन: {0}', ja:'最終スキャン: {0}', ky:'Акыркы скандоо: {0}', 'zh-Hant':'上次掃描：{0}' },
...
async function scanTarget(st, tg) {
  ...
  save(st);
  if (st.notify && hasChanges(diff)) notify(t('pw_notif_title'), tname(tg) + ' — ' + summary(diff));
  return diff;
}
    ...
    body.innerHTML = `
      ...
      <div id="pw-list">${st.targets.map((tg) => { const last = tg.history[tg.history.length - 1]; return `
        <div class="pw-card" data-id="${esc(tg.id)}"><div><b>${esc(tname(tg))}</b>${tg.sample ? `<span class="pw-badge">${esc(t('pw_sample_badge'))}</span>` : ''}</div>
          ...
          <div class="pw-sum">${last ? `${esc(tf('pw_last', fmtTs(last.ts)))}<br><b>${esc(entrySummary(last))}</b>` : esc(t('pw_never'))}</div>
