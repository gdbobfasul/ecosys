# Android форма — aip1-plainaction
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-plainaction-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-plainaction/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Paste or type any document into the text box (or tap an example) and press the analyze/simplify action to see a plain-language summary and a checklist of what to do; this works offline with built-in logic. To try the optional AI-assisted mode, open the gear/settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the document text is sent to that provider only when you run the action. History is stored locally and can be cleared. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-plainaction-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1plainaction.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-plainaction/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
