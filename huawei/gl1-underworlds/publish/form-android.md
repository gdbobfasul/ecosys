...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-underworlds-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-underworlds/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, use the left/right buttons to move, up to climb or act, and the action button to use the highlighted tool; dig downward and pick the right tool for each obstacle (pickaxe and shovel for stone, drill for walls, heat suit for lava, detonator for gunpowder, pump for flooded tunnels, rope for chasms, lamp for the dark) to reach the glowing core at the bottom. Every level is solvable. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-underworlds-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1underworlds.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-underworlds/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
