...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-mediator-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-mediator/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Type each side of the disagreement into the two text fields and press the mediate button to see a neutral summary, each side's needs, common ground and next steps; this works offline with built-in guidance. To try the optional AI-assisted mode, open the gear/settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the text is sent to that provider only when you press the mediate button. History is stored locally and can be cleared. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-mediator-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1mediator.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-mediator/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
