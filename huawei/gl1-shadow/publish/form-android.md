...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-shadow-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-shadow/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, tap JUMP to leap over rocks and pits and hold DUCK to pass under beams — you control only the shadow; the man above is hurt when the shadow fails an obstacle. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-shadow-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1shadow.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-shadow/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
