# Публикуване в Huawei AppGallery — NewsLator
...
- **Huawei пакет:** `com.pupikes.newslator.hw`
- **Билд (APK/AAB):** `apk/huawei/release/NewsLator-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.newslator.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.newslator.hw`** (фиксира се веднъж — точно това).
...
| Текстове по език | суровите описания за всеки език (16 файла) | [`store-listing/`](./store-listing/) |
...
| Екранни снимки | поне 3; споделени + по език (16 папки/файла) | [`screenshots/`](./screenshots/) |
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/newslator/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „News & reading" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | NewsLator |
| bg | Български | **Bulgarian** | NewsLator — преводач на новини |
| ru | Русский | **Russian** | NewsLator — Новости на вашем языке |
| uk | Українська | **Ukrainian** | NewsLator |
| de | Deutsch | **German** | NewsLator |
| fr | Français | **French (France)** | NewsLator |
| es | Español | **Spanish (Spain)** | NewsLator |
| es-MX | Español (MX) | **Spanish (Latin America)** | NewsLator |
| it | Italiano | **Italian** | NewsLator |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | NewsLator |
| ar | العربية | **Arabic** | NewsLator — أخبار العالم |
| hi | हिन्दी | **Hindi** | NewsLator — विश्व समाचार |
| ja | 日本語 | **Japanese** | NewsLator — 世界のニュース |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | NewsLator — 世界新聞 |
...
| **Screenshots** (задължително) | екраните от `screenshots/` (поне 3) — налични 16. Препоръка: избор на език, интро, съгласие, начален екран |
...
|---|---|
| 1 (тип) | **Apps** |
| 2 (категория) | **News & reading** |
| 3 (под-категория) | **News** |

**Пълен път:** `Apps > News & reading > News`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/newslator/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/NewsLator-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/newslator/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
