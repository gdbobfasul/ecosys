# Документи за публикуване — NewsLator
...
- **RuStore пакет:** `com.pupikes.newslator.rustore`
- **Билд (APK/AAB):** идва от `apk/newslator-{huawei,rustore}-debug.apk` след меню 57 (не е в тази папка).
...
> Пакетът вече е `com.pupikes.newslator.hw` (сменен от стария `com.kcy.newslator.hw`). AppGallery **не позволява** смяна на
> пакета на съществуващ запис — ако качиш в стария, отказва с „_the name of the uploaded package is
> different from the existing package name com.kcy.newslator.hw_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.newslator.hw`** (фиксира се веднъж — трябва да е точно това).
> 2. Попълни данните по таблицата долу и качи APK-то в **новия** запис.
> 3. Ако старият `com.kcy.newslator.hw` е бил публикуван — свали го от продажба СЛЕД одобрение на новия.
...
> Пакетът вече е `com.pupikes.newslator.rustore` (сменен от стария `com.kcy.newslator.rustore`). RuStore, както и Huawei, **не позволява**
> смяна на пакета на съществуващ запис. Затова: създай **ново приложение** с
> **applicationId = `com.pupikes.newslator.rustore`** и качи APK-то там. Ако старият `com.kcy.newslator.rustore` е бил публикуван — свали
...
| **Политика за поверителност (руски)** | подава се като URL в RuStore Console + в апа | [`ru-privacy.html`](./ru-privacy.html) → `https://selflearning.bot.nu/privacy/newslator/ru-privacy.html` |
...
_Политики онлайн: Huawei → `https://selflearning.bot.nu/privacy/newslator/hw-privacy.html` · RuStore → `https://selflearning.bot.nu/privacy/newslator/ru-privacy.html` (качват се на сървъра чрез меню 08 — sync_privacy_pages)._
