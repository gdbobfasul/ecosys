# Публикуване в RuStore — Pupikes Market Pulse
...
- **Билд (APK/AAB):** `apk/rustore/release/Pupikes-Market-Pulse-rustore-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.marketpulse.rustore`. RuStore, както Huawei, **не позволява** смяна на пакета на съществуващ запис.
> Създай **ново приложение** с **applicationId = `com.pupikes.marketpulse.rustore`** и качи APK-то там. Ако е съществувал стар
...
| Поле | Стойност |
|---|---|
| applicationId | `com.pupikes.marketpulse.rustore` |
| Название приложения | `Pupikes Market Pulse` |
...
| Скриншоты | `screenshots/` (1–10) — налични 14 |

### 4. Разрешения и данни
- **Политика конфиденциальности (URL):** `https://selflearning.bot.nu/privacy/market-pulse/rustore-privacy.html` (адрес; същата се отваря и в апа).
...
| Поле | Стойност |
|---|---|
| APK/AAB | качи `apk/rustore/release/Pupikes-Market-Pulse-rustore-release.apk` |

### 7. Преди подаване
Прегледай `analyse.rustore` → **Отправить на модерацию**.

_Политика онлайн (RuStore): `https://selflearning.bot.nu/privacy/market-pulse/rustore-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
