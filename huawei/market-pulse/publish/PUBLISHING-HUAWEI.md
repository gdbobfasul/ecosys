# Публикуване в Huawei AppGallery — Pupikes Market Pulse
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Market-Pulse-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.marketpulse.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.marketpulse.hw`** (фиксира се веднъж — точно това).
...
| Екранни снимки | поне 3; споделени + по език (14 папки/файла) | [`screenshots/`](./screenshots/) |
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/market-pulse/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „Finance" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Market Pulse |
| bg | Български | **Bulgarian** | Pupikes Market Pulse |
| ru | Русский | **Russian** | Pupikes Market Pulse |
| uk | Українська | **Ukrainian** | Pupikes Market Pulse |
| de | Deutsch | **German** | Pupikes Market Pulse |
| fr | Français | **French (France)** | Pupikes Market Pulse |
| es | Español | **Spanish (Spain)** | Pupikes Market Pulse |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Market Pulse |
| it | Italiano | **Italian** | Pupikes Market Pulse |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Market Pulse |
| ar | العربية | **Arabic** | Pupikes Market Pulse |
| hi | हिन्दी | **Hindi** | Pupikes Market Pulse |
| ja | 日本語 | **Japanese** | Pupikes Market Pulse |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Market Pulse |
...
| **Screenshots** (задължително) | екраните от `screenshots/` (поне 3) — налични 14. Препоръка: избор на език, интро, съгласие, начален екран |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/market-pulse/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Market-Pulse-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/market-pulse/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
