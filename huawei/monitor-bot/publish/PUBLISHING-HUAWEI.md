# Публикуване в Huawei AppGallery — Pupikes Site Monitor
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Site-Monitor-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.monitorbot.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.monitorbot.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/monitor-bot/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Site Monitor |
| bg | Български | **Bulgarian** | Pupikes Site Monitor |
| ru | Русский | **Russian** | Pupikes Site Monitor |
| uk | Українська | **Ukrainian** | Pupikes Site Monitor |
| de | Deutsch | **German** | Pupikes Site Monitor |
| fr | Français | **French (France)** | Pupikes Site Monitor |
| es | Español | **Spanish (Spain)** | Pupikes Site Monitor |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Site Monitor |
| it | Italiano | **Italian** | Pupikes Site Monitor |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Site Monitor |
| ar | العربية | **Arabic** | Pupikes Site Monitor |
| hi | हिन्दी | **Hindi** | Pupikes Site Monitor |
| ja | 日本語 | **Japanese** | Pupikes Site Monitor |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Site Monitor |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/monitor-bot/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Site-Monitor-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/monitor-bot/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
