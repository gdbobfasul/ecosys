...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-bloodstream-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-bloodstream/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, drag on the screen to select your defensive cells (or tap the central heart to select all), then tap an infection focus and the selected cells travel along the veins to attack it. Use the on-screen Boost and Heal buttons when unlocked. Destroy all foci before the body's integrity reaches zero to clear the level. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-bloodstream-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1bloodstream.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-bloodstream/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
