...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-flock-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-flock/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, use the on-screen direction pads to steer your flock: lead the birds toward the destination, keep the formation together, and avoid predators and obstacles to clear each level. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-flock-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1flock.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-flock/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
