# Публикуване в Huawei AppGallery — MotionHawk
...
- **Huawei пакет:** `com.pupikes.camerawatch.hw`
- **Билд (APK/AAB):** `apk/huawei/release/MotionHawk-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.camerawatch.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.camerawatch.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/camera-watch/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | MotionHawk |
| bg | Български | **Bulgarian** | MotionHawk |
| ru | Русский | **Russian** | MotionHawk |
| uk | Українська | **Ukrainian** | MotionHawk |
| de | Deutsch | **German** | MotionHawk |
| fr | Français | **French (France)** | MotionHawk |
| es | Español | **Spanish (Spain)** | MotionHawk |
| es-MX | Español (MX) | **Spanish (Latin America)** | MotionHawk |
| it | Italiano | **Italian** | MotionHawk |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | MotionHawk |
| ar | العربية | **Arabic** | MotionHawk |
| hi | हिन्दी | **Hindi** | MotionHawk |
| ja | 日本語 | **Japanese** | MotionHawk |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | MotionHawk |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/camera-watch/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/MotionHawk-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/camera-watch/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
