...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/camera-watch-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/camera-watch/hw-privacy.html` |
...
```
No account or login is required. On first launch pick a language, read the onboarding, and tap 'Activate for free', then continue to Permissions and grant Camera (and optionally Notifications). Start the watcher and move in front of the camera; the app detects motion, labels what moved, fires a local notification, and adds a snapshot to the log. All processing happens on-device. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
