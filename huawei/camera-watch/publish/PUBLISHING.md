# Документи за публикуване — MotionHawk
...
- **RuStore пакет:** `com.pupikes.camerawatch.rustore`
- **Билд (APK/AAB):** идва от `apk/camera-watch-{huawei,rustore}-debug.apk` след меню 57 (не е в тази папка).
...
> Пакетът вече е `com.pupikes.camerawatch.hw` (сменен от стария `com.kcy.camerawatch.hw`). AppGallery **не позволява** смяна на
> пакета на съществуващ запис — ако качиш в стария, отказва с „_the name of the uploaded package is
> different from the existing package name com.kcy.camerawatch.hw_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.camerawatch.hw`** (фиксира се веднъж — трябва да е точно това).
> 2. Попълни данните по таблицата долу и качи APK-то в **новия** запис.
> 3. Ако старият `com.kcy.camerawatch.hw` е бил публикуван — свали го от продажба СЛЕД одобрение на новия.
...
> Пакетът вече е `com.pupikes.camerawatch.rustore` (сменен от стария `com.kcy.camerawatch.rustore`). RuStore, както и Huawei, **не позволява**
> смяна на пакета на съществуващ запис. Затова: създай **ново приложение** с
> **applicationId = `com.pupikes.camerawatch.rustore`** и качи APK-то там. Ако старият `com.kcy.camerawatch.rustore` е бил публикуван — свали
...
| **Политика за поверителност (руски)** | подава се като URL в RuStore Console + в апа | [`rustore-privacy.html`](./rustore-privacy.html) → `https://selflearning.bot.nu/privacy/camera-watch/rustore-privacy.html` |
...
_Политики онлайн: Huawei → `https://selflearning.bot.nu/privacy/camera-watch/hw-privacy.html` · RuStore → `https://selflearning.bot.nu/privacy/camera-watch/rustore-privacy.html` (качват се на сървъра чрез меню 08 — sync_privacy_pages)._
