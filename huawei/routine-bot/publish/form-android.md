...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/routine-bot-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/routine-bot/hw-privacy.html` |
...
```
No account or login is required. On first launch pick a language and activate the robot, then grant the notification permission. Add reminders (with time and days), notes, and optionally type a city name to enable the weather part of the morning briefing. Use the briefing preview to see it immediately. Internet is used only for weather lookups by city name; location/GPS is never accessed. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
