# Публикуване в Huawei AppGallery — Pupikes Routine Planner
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Routine-Planner-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.routinebot.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.routinebot.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/routine-bot/hw-privacy.html` |
...
| App name | `Pupikes Routine Planner` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Routine Planner |
| bg | Български | **Bulgarian** | Pupikes Routine Planner |
| ru | Русский | **Russian** | Pupikes Routine Planner |
| uk | Українська | **Ukrainian** | Pupikes Routine Planner |
| de | Deutsch | **German** | Pupikes Routine Planner |
| fr | Français | **French (France)** | Pupikes Routine Planner |
| es | Español | **Spanish (Spain)** | Pupikes Routine Planner |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Routine Planner |
| it | Italiano | **Italian** | Pupikes Routine Planner |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Routine Planner |
| ar | العربية | **Arabic** | Pupikes Routine Planner |
| hi | हिन्दी | **Hindi** | Pupikes Routine Planner |
| ja | 日本語 | **Japanese** | Pupikes Routine Planner |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Routine Planner |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/routine-bot/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Routine-Planner-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/routine-bot/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
