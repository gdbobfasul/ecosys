...
function modelName() {
  try { const m = localStorage.getItem('slf.whisper.model'); if (m === 'tiny') return 'Xenova/whisper-tiny'; if (m === 'base') return 'Xenova/whisper-base'; if (m === 'small') return 'Xenova/whisper-small'; } catch (_) {}
  return 'Xenova/whisper-small';   // ПО ПОДРАЗБИРАНЕ small — по-точно разпознаване (по-голям модел ~240MB, по-бавно, но по-вярно)
}
...
export async function startWhisper({ lang = 'bg', onInterim = null, manualStop = false, onStatus = null, onState = null, maxMs = 600000 } = {}) {
  if (_listening) throw new Error('busy');
  _listening = true; _aborted = false; _chunks = [];
  const state = (s) => { if (onState) { try { onState(s); } catch (_) {} } };
  state('preparing');
  ...
  catch (e) { _listening = false; state('error'); throw new Error('denied'); }
  ...
  } catch (e) { _listening = false; state('error'); try { _stream.getTracks().forEach((t) => t.stop()); } catch (_) {} throw new Error('whisper-load'); }
  ...
  _rec.start();
  state('recording');   // ★ ОТ ТУК записва → покажи „Говори"
  ...
  state('transcribing');   // ★ след стоп (втория сигнал) → покажи „Изчакай"
