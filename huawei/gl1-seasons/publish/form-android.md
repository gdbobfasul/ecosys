...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-seasons-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-seasons/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, hold the on-screen buttons to march forward and jump, and press the season button to switch between winter, spring, summer and autumn (the number of switches is limited). Change the season to open each blocked gate and to drive off the beasts, avoid avalanches, floods and fire, and reach the tower to clear the level. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-seasons-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1seasons.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-seasons/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
