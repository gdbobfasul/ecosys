...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-skillcoach-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-skillcoach/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Choose one of the built-in skills (or type your own) and tap to build a 6-day program; each day shows a short task with a done-criterion you can mark complete and rate for difficulty. This works offline using built-in programs. To generate a custom program for any skill, open Settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the skill name and notes are sent to that provider only on your explicit action. Progress is stored locally and can be reset. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-skillcoach-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1skillcoach.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-skillcoach/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
