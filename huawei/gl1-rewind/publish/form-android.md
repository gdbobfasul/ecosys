...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-rewind-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-rewind/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, hold the on-screen buttons to walk back, jump and fire in reverse; every shot un-fires and the fallen figures you reach revive, closing the time rift. Reach the start of the street to clear the level. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-rewind-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1rewind.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-rewind/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
