...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/titans-fight-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/titans-fight/hw-privacy.html` |
...
```
No account or internet is required. On first launch pick a language, then tap a level in the menu to start; choose a weapon and press Fight. Win a match to unlock the next level, and optionally enter a name to save a score to the on-device leaderboard. The app requests no runtime permissions. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
