# Публикуване в Huawei AppGallery — Auto Sound Diagnostics
...
- **Huawei пакет:** `com.pupikes.autosound.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Auto-Sound-Diagnostics-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.autosound.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.autosound.hw`** (фиксира се веднъж — точно това).
...
| Екранни снимки | поне 3; споделени + по език (5 папки/файла) | [`screenshots/`](./screenshots/) |
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/hw-privacy.html` |
...
| App name | `Auto Sound Diagnostics` |
...
→ **OK** _(съдържателната категория „Cars" се задава по-късно — на екрана Categorization/App information, не тук)_
...
#### Manage languages — езиците за ТОВА приложение (3 наши → 4 записа в Huawei)
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Auto Sound Diagnostics |
| bg | Български | **Bulgarian** | Auto Sound Diagnostics |
| ru | Русский | **Russian** | Auto Sound Diagnostics |
...
- **Не се избират:** uk (не е преведен), de (не е преведен), fr (не е преведен), es (не е преведен), es-MX (не е преведен), it (не е преведен), pt (не е преведен), ar (не е преведен), hi (не е преведен), ja (не е преведен), ky (няма в Huawei), zh-Hant (не е преведен).
...
| **Screenshots** (задължително) | екраните от `screenshots/` (поне 3) — налични 5. Препоръка: избор на език, интро, съгласие, начален екран |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Auto-Sound-Diagnostics-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
