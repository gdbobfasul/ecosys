# Публикуване в RuStore — Auto Sound Diagnostics

_Автоматичен индекс (deploy-scripts/gen-publish-index.mjs). САМО за RuStore. За Huawei виж `PUBLISHING-HUAWEI.md`._

- **RuStore пакет:** `com.pupikes.autosound.rustore`
- **Билд (APK/AAB):** `apk/rustore/release/Auto-Sound-Diagnostics-rustore-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.autosound.rustore`. RuStore, както Huawei, **не позволява** смяна на пакета на съществуващ запис.
> Създай **ново приложение** с **applicationId = `com.pupikes.autosound.rustore`** и качи APK-то там. Ако е съществувал стар
...
| **Политика за поверителност (руски)** | подава се като URL в RuStore Console + в апа | [`rustore-privacy.html`](./rustore-privacy.html) → `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/rustore-privacy.html` |
...
| Поле | Стойност |
|---|---|
| applicationId | `com.pupikes.autosound.rustore` |
| Название приложения | `Auto Sound Diagnostics` |
| Категория | Авто |
...
| Скриншоты | `screenshots/` (1–10) — налични 5 |

### 4. Разрешения и данни
- **Политика конфиденциальности (URL):** `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/rustore-privacy.html` (адрес; същата се отваря и в апа).
...
| Поле | Стойност |
|---|---|
| APK/AAB | качи `apk/rustore/release/Auto-Sound-Diagnostics-rustore-release.apk` |

### 7. Преди подаване
Прегледай `analyse.rustore` → **Отправить на модерацию**.

_Политика онлайн (RuStore): `https://selflearning.bot.nu/privacy/auto-sound-diagnostics/rustore-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
