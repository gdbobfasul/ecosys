# Публикуване в Huawei AppGallery — Pupikes Toolkit Price Watch
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Price-Watch-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.pricewatchbot.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.pricewatchbot.hw`** (фиксира се веднъж — точно това).
...
| App name | `Pupikes Toolkit Price Watch` |
...
→ **OK** _(съдържателната категория „Shopping" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Price Watch |
| bg | Български | **Bulgarian** | Pupikes Toolkit Price Watch |
| ru | Русский | **Russian** | Pupikes Toolkit Price Watch |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Price Watch |
| de | Deutsch | **German** | Pupikes Toolkit Price Watch |
| fr | Français | **French (France)** | Pupikes Toolkit Price Watch |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Price Watch |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Price Watch |
| it | Italiano | **Italian** | Pupikes Toolkit Price Watch |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Price Watch |
| ar | العربية | **Arabic** | Pupikes Toolkit Price Watch |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Price Watch |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Price Watch |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Price Watch |
...
|---|---|
| 1 (тип) | **Apps** |
| 2 (категория) | **Shopping** |
| 3 (под-категория) | **Discounts** |

**Пълен път:** `Apps > Shopping > Discounts`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/price-watch-bot/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Price-Watch-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/price-watch-bot/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
