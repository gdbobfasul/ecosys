# Публикуване в Huawei AppGallery — Pupikes Doctor
...
- **Huawei пакет:** `com.pupikes.doctor.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Doctor-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.doctor.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.doctor.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-doctor/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Doctor |
| bg | Български | **Bulgarian** | Pupikes Doctor |
| ru | Русский | **Russian** | Pupikes Doctor |
| uk | Українська | **Ukrainian** | Pupikes Doctor |
| de | Deutsch | **German** | Pupikes Doctor |
| fr | Français | **French (France)** | Pupikes Doctor |
| es | Español | **Spanish (Spain)** | Pupikes Doctor |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Doctor |
| it | Italiano | **Italian** | Pupikes Doctor |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Doctor |
| ar | العربية | **Arabic** | Pupikes Doctor |
| hi | हिन्दी | **Hindi** | Pupikes Doctor |
| ja | 日本語 | **Japanese** | Pupikes Doctor |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Doctor |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-doctor/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Doctor-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-doctor/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
