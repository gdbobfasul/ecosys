...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/baby-monitor-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/baby-monitor/hw-privacy.html` |
...
```
No account or login is required. On first launch pick a language, read the onboarding (including the safety disclaimer that it is not a certified baby monitor), and continue to Permissions. Grant Camera (and optionally Notifications), point the phone camera at a scene, and start the watcher; moving in front of the camera triggers a local alert and a logged snapshot. Everything runs on-device. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
