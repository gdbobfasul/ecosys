...
**Huawei AppGallery** (устройствата нямат GMS; при нужда се ползва HMS, не GMS).
...
Mobile Services са в `src/store/huawei-sdk.js` (стъбове). Тестова/sandbox
среда — `store/TESTING.md`. При добавяне на HMS трябва и
`agconnect-services.json` от AppGallery Connect в android проекта.

## Структура
```
huawei/plane-shooter/
├─ index.html
├─ package.json
├─ vite.config.js
...
│  ├─ theme.js                  # цветова тема (топла, различава се от RUStore)
│  ├─ gfx/
