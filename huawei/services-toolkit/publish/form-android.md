# Android форма — services-toolkit
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/services-toolkit-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/services-toolkit/hw-privacy.html` |
...
```
No account needed. On first launch choose a language and tap Start to reach the tool grid, then open any tool. Offline tools work with no connection; online tools (currency, crypto, scraper, AI text) need internet. The QR tool asks for camera permission only when you start scanning, and the currency watcher may request notification permission for alerts. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
