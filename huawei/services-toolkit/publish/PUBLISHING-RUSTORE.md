# Публикуване в RuStore — Pupikes Toolkit
...
- **Билд (APK/AAB):** `apk/rustore/release/Pupikes-Toolkit-rustore-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.servicestoolkit.rustore`. RuStore, както Huawei, **не позволява** смяна на пакета на съществуващ запис.
> Създай **ново приложение** с **applicationId = `com.pupikes.servicestoolkit.rustore`** и качи APK-то там. Ако е съществувал стар
...
| Поле | Стойност |
|---|---|
| applicationId | `com.pupikes.servicestoolkit.rustore` |
| Название приложения | `Pupikes Toolkit` |
...
- **Политика конфиденциальности (URL):** `https://selflearning.bot.nu/privacy/services-toolkit/rustore-privacy.html` (адрес; същата се отваря и в апа).
...
| Поле | Стойност |
|---|---|
| APK/AAB | качи `apk/rustore/release/Pupikes-Toolkit-rustore-release.apk` |

### 7. Преди подаване
Прегледай `analyse.rustore` → **Отправить на модерацию**.

_Политика онлайн (RuStore): `https://selflearning.bot.nu/privacy/services-toolkit/rustore-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
