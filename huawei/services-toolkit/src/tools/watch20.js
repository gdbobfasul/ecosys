...
// Дефинира се с динамичен (runtime) специфакатор, за да НЕ се опитва Vite да
// резолва пакета по време на билд (services-toolkit не зависи от плъгина).
// ─────────────────────────────────────────────────────────────────────────
...
async function getLocalNotifications() {
  ...
  try {
    ...
    if (core && core.Capacitor && core.Capacitor.isNativePlatform()) {
      // runtime специфакатор → Vite не го резолва статично (плъгинът е опционален)
      const pkg = ['@capacitor', 'local-notifications'].join('/');
      const mod = await import(/* @vite-ignore */ pkg);
      _ln = (mod && mod.LocalNotifications) || false;
    } else {
      _ln = false;
    }
  } catch (_) {
...
export function render(root) {
  ...
  function renderGrid() {
    ...
    for (let i = 0; i < SLOTS; i++) {
      ...
      card.className = 'tool-card' + (s._alerted ? ' w20-alerted' : '');
      ...
      card.innerHTML = `
        ...
        <div style="font-size:1.3em;font-weight:700;margin:16px 0 2px;color:var(--text)">${fmtRate(rate)}</div>
        <div style="font-size:.78em;color:var(--text-dim);min-height:1em">${s.sel ? selLabel(s.sel) : 'не е избрана'}</div>
  ...
  function renderThrList() {
    ...
    box.querySelectorAll('.w20DelThr').forEach((b) => {
      b.addEventListener('click', () => { slots[openModalIdx].alerts.splice(+b.dataset.i, 1); saveSlots(slots); renderThrList(); renderGrid(); });
    });
  }
  ...
  async function refreshAll() {
    ...
    if (!sels.length) { setStatus('work', 'Избери поне една двойка, за да започне следенето.'); renderGrid(); return; }
