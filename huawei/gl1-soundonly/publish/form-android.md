...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-soundonly-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-soundonly/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. Turn the device sound on: each enemy reveals its position only with a fading sound pulse. Tap/click the centre of the sound to strike, press the left/right buttons right after to dodge the counter, and use up/down to change distance. Defeat the enemies to advance through the levels. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-soundonly-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1soundonly.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-soundonly/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
