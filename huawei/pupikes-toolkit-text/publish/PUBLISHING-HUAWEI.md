# Публикуване в Huawei AppGallery — Pupikes Toolkit Text
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Text-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkittext.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkittext.hw`** (фиксира се веднъж — точно това).
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Text |
| bg | Български | **Bulgarian** | Pupikes Toolkit Text |
| ru | Русский | **Russian** | Pupikes Toolkit Text |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Text |
| de | Deutsch | **German** | Pupikes Toolkit Text |
| fr | Français | **French (France)** | Pupikes Toolkit Text |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Text |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Text |
| it | Italiano | **Italian** | Pupikes Toolkit Text |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Text |
| ar | العربية | **Arabic** | Pupikes Toolkit Text |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Text |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Text |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Text |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-text/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Text-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-text/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
