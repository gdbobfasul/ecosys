# Публикуване в Huawei AppGallery — Huntline 3D
...
- **Билд (APK/AAB):** `apk/huawei/release/Huntline-3D-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.fpshunter.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.fpshunter.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/fps-hunter/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Huntline 3D |
| bg | Български | **Bulgarian** | Huntline 3D |
| ru | Русский | **Russian** | Huntline 3D |
| uk | Українська | **Ukrainian** | Huntline 3D |
| de | Deutsch | **German** | Huntline 3D |
| fr | Français | **French (France)** | Huntline 3D |
| es | Español | **Spanish (Spain)** | Huntline 3D |
| es-MX | Español (MX) | **Spanish (Latin America)** | Huntline 3D |
| it | Italiano | **Italian** | Huntline 3D |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Huntline 3D |
| ar | العربية | **Arabic** | Huntline 3D |
| hi | हिन्दी | **Hindi** | Huntline 3D |
| ja | 日本語 | **Japanese** | Huntline 3D |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Huntline 3D |
...
| 3 (под-категория) | **Shooting** |

**Пълен път:** `Games > Action Games > Shooting`  _(игра — top ниво Games)_
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/fps-hunter/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Huntline-3D-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/fps-hunter/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
