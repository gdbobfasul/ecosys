...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-gravity-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-gravity/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game, hold the left/right buttons to rotate your craft and press GAS to thrust; use each planet's gravity to slingshot toward the next planet and clear the level. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-gravity-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1gravity.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-gravity/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
