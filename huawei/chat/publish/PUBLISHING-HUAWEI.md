# Публикуване в Huawei AppGallery — Pupikes Chat
...
- **Huawei пакет:** `com.pupikes.chat.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Chat-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.chat.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.chat.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/chat/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „Social" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Chat |
| bg | Български | **Bulgarian** | Pupikes Chat |
| ru | Русский | **Russian** | Pupikes Chat |
| uk | Українська | **Ukrainian** | Pupikes Chat |
| de | Deutsch | **German** | Pupikes Chat |
| fr | Français | **French (France)** | Pupikes Chat |
| es | Español | **Spanish (Spain)** | Pupikes Chat |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Chat |
| it | Italiano | **Italian** | Pupikes Chat |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Chat |
| ar | العربية | **Arabic** | Pupikes Chat |
| hi | हिन्दी | **Hindi** | Pupikes Chat |
| ja | 日本語 | **Japanese** | Pupikes Chat |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Chat |
...
| 1 (тип) | **Apps** |
| 2 (категория) | **Social** |
| 3 (под-категория) | **Chatting** |

**Пълен път:** `Apps > Social > Chatting`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/chat/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Chat-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/chat/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
