...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/chat-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/chat/hw-privacy.html` |
| Data subject right URL | празно |
| Collect personal data | **Yes** |
...
| Sign-in required | отметни (има вход) |
| Release time | Immediately once approved |

**For reviewer → Remarks** (копирай):
```
Launch the app; on first run choose a language and tap Start. It connects to the live chat server over the internet, so a working network connection is required. Account/registration steps happen inside the chat itself; no credentials are needed to reach the language screen and connect. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
