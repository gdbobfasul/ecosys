# Публикуване в RuStore — Pupikes Learning Buddy
...
- **Билд (APK/AAB):** `apk/rustore/release/Pupikes-Learning-Buddy-rustore-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.selflearningfriend.rustore`. RuStore, както Huawei, **не позволява** смяна на пакета на съществуващ запис.
> Създай **ново приложение** с **applicationId = `com.pupikes.selflearningfriend.rustore`** и качи APK-то там. Ако е съществувал стар
...
| Поле | Стойност |
|---|---|
| applicationId | `com.pupikes.selflearningfriend.rustore` |
| Название приложения | `Pupikes LB` |
...
- **Политика конфиденциальности (URL):** `https://selflearning.bot.nu/privacy/selflearning-friend/rustore-privacy.html` (адрес; същата се отваря и в апа).
- Разрешения (обосновка за модерацията):
- **Микрофон (RECORD_AUDIO)** — виж app-profile.json
- **Камера (CAMERA)** — по функцията на приложението
- Сеть: да — только технические запросы, описаны в политике.
...
| Поле | Стойност |
|---|---|
| APK/AAB | качи `apk/rustore/release/Pupikes-Learning-Buddy-rustore-release.apk` |

### 7. Преди подаване
Прегледай `analyse.rustore` → **Отправить на модерацию**.

_Политика онлайн (RuStore): `https://selflearning.bot.nu/privacy/selflearning-friend/rustore-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
