# Самообучаващ се приятел — описание по език (Brief + Full + New features)

_За AppGallery: **Manage languages** → добави език (по колоната „AppGallery език") → попълни **Brief introduction** (до 25 знака), **Full introduction** и **New features** от секцията за съответния език._

> ⚠️ **ky (кыргызки) НЕ съществува в AppGallery** — пропусни го (14 от 15 се поддържат). Махни отметките от **Greek** и **Turkish**, ако са чекнати (нямаме преводи за тях).

## Български (bg) — AppGallery: **Bulgarian**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Аз съм твоят личен приятел. Ще се уча само от теб и ще принадлежа само на теб. Първо ми избери кодова дума (РАЗКОВНИЧЕ) — тя е нашата тайна за достъп.

Ще съм твой личен асистент: помня името ти, навиците, предпочитанията — всичко за теб, само на това устройство.

НЕ събирам нищо лично за теб. Учиш ме на общи знания, ставам умен и после можеш да ме продадеш/прехвърлиш „чист“. Новият собственик, ако иска, ще ми разказва за себе си.

Поддръжка: dai.group.ltd.support@gmail.com
За въпроси и поддръжка пишете на: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Русский (ru) — AppGallery: **Russian**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Я твой личный друг. Я буду учиться только у тебя и принадлежать только тебе. Сначала выбери мне кодовое слово — это наш секрет доступа.

Я буду твоим личным ассистентом: помню твоё имя, привычки, предпочтения — всё о тебе, только на этом устройстве.

Я НЕ собираю ничего личного о тебе. Ты учишь меня общим знаниям, я становлюсь умным, и потом ты можешь продать/передать меня «чистым». Новый владелец, если захочет, расскажет мне о себе.

Поддержка: dai.group.ltd.support@gmail.com
По вопросам и поддержке пишите на: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Українська (uk) — AppGallery: **Ukrainian**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Я твій особистий друг. Я навчатимуся лише в тебе і належатиму лише тобі. Спершу обери мені кодове слово — це наш секрет доступу.

Я буду твоїм особистим асистентом: пам’ятаю твоє ім’я, звички, уподобання — усе про тебе, лише на цьому пристрої.

Я НЕ збираю нічого особистого про тебе. Ти вчиш мене загальних знань, я розумнішаю, і потім ти можеш продати/передати мене «чистим». Новий власник, якщо схоче, розповість мені про себе.

Підтримка: dai.group.ltd.support@gmail.com
З питань і підтримки пишіть на: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## English (en) — AppGallery: **English (UK) — default**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

I am your personal friend. I will learn only from you and belong only to you. First, choose a code word for me — it is our secret for access.

I will be your personal assistant: I remember your name, habits, preferences — everything about you, only on this device.

I collect NOTHING personal about you. You teach me general knowledge, I get smart, and then you can sell/transfer me "clean". The new owner, if they wish, will tell me about themselves.

Support: dai.group.ltd.support@gmail.com
For questions and support, write to: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Deutsch (de) — AppGallery: **German**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Ich bin dein persönlicher Freund. Ich lerne nur von dir und gehöre nur dir. Wähle zuerst ein Codewort für mich — es ist unser Zugangsgeheimnis.

Ich bin dein persönlicher Assistent: Ich merke mir deinen Namen, Gewohnheiten, Vorlieben — alles über dich, nur auf diesem Gerät.

Ich sammle NICHTS Persönliches über dich. Du bringst mir Allgemeinwissen bei, ich werde klug, und dann kannst du mich „sauber" verkaufen/übergeben. Der neue Besitzer erzählt mir, wenn er möchte, von sich.

Support: dai.group.ltd.support@gmail.com
Bei Fragen und für Support schreiben Sie an: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Français (fr) — AppGallery: **French (France)**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Je suis ton ami personnel. Je n’apprendrai que de toi et n’appartiendrai qu’à toi. Choisis d’abord un mot de passe pour moi — c’est notre secret d’accès.

Je serai ton assistant personnel : je retiens ton nom, tes habitudes, tes préférences — tout sur toi, uniquement sur cet appareil.

Je ne collecte RIEN de personnel sur toi. Tu m’enseignes des connaissances générales, je deviens intelligent, puis tu peux me vendre/transférer « propre ». Le nouveau propriétaire, s’il veut, me parlera de lui.

Assistance: dai.group.ltd.support@gmail.com
Pour toute question ou assistance, écrivez à : dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Español (es) — AppGallery: **Spanish (Spain)**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Soy tu amigo personal. Aprenderé solo de ti y te perteneceré solo a ti. Primero elige una palabra clave para mí: es nuestro secreto de acceso.

Seré tu asistente personal: recuerdo tu nombre, hábitos, preferencias — todo sobre ti, solo en este dispositivo.

NO recopilo nada personal sobre ti. Me enseñas conocimientos generales, me vuelvo inteligente y luego puedes venderme/transferirme «limpio». El nuevo dueño, si quiere, me hablará de sí mismo.

Soporte: dai.group.ltd.support@gmail.com
Para preguntas y soporte, escribe a: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Español (MX) (es-MX) — AppGallery: **Spanish (Latin America)**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Soy tu amigo personal. Aprenderé solo de ti y te perteneceré solo a ti. Primero elige una palabra clave para mí: es nuestro secreto de acceso.

Seré tu asistente personal: recuerdo tu nombre, hábitos, preferencias — todo sobre ti, solo en este dispositivo.

NO recopilo nada personal sobre ti. Me enseñas conocimientos generales, me vuelvo inteligente y luego puedes venderme/transferirme «limpio». El nuevo dueño, si quiere, me hablará de sí mismo.

Soporte: dai.group.ltd.support@gmail.com
Para dudas y soporte, escribe a: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Italiano (it) — AppGallery: **Italian**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Sono il tuo amico personale. Imparerò solo da te e apparterrò solo a te. Prima scegli una parola chiave per me — è il nostro segreto di accesso.

Sarò il tuo assistente personale: ricordo il tuo nome, abitudini, preferenze — tutto su di te, solo su questo dispositivo.

NON raccolgo nulla di personale su di te. Mi insegni nozioni generali, divento intelligente, e poi puoi vendermi/trasferirmi «pulito». Il nuovo proprietario, se vuole, mi parlerà di sé.

Assistenza: dai.group.ltd.support@gmail.com
Per domande e assistenza, scrivi a: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## Português (pt) — AppGallery: **Portuguese (Portugal)**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

Sou seu amigo pessoal. Vou aprender só com você e pertencer só a você. Primeiro escolha uma palavra-chave para mim — é nosso segredo de acesso.

Serei seu assistente pessoal: lembro seu nome, hábitos, preferências — tudo sobre você, só neste dispositivo.

NÃO coleto nada pessoal sobre você. Você me ensina conhecimento geral, fico inteligente, e depois pode me vender/transferir "limpo". O novo dono, se quiser, me contará sobre si.

Suporte: dai.group.ltd.support@gmail.com
Para dúvidas e suporte, escreva para: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## العربية (ar) — AppGallery: **Arabic**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

أنا صديقك الشخصي. سأتعلّم منك فقط وأنتمي إليك وحدك. اختر أولًا كلمة سر لي — إنها سرّ الوصول بيننا.

سأكون مساعدك الشخصي: أتذكّر اسمك وعاداتك وتفضيلاتك — كل شيء عنك، على هذا الجهاز فقط.

لا أجمع أي شيء شخصي عنك. تعلّمني المعارف العامة فأصبح ذكيًا، ثم يمكنك بيعي/نقلي «نظيفًا». المالك الجديد، إن أراد، سيحدثني عن نفسه.

الدعم: dai.group.ltd.support@gmail.com
للأسئلة والدعم، راسلونا على: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## हिन्दी (hi) — AppGallery: **Hindi**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

मैं तुम्हारा निजी दोस्त हूँ। मैं केवल तुमसे सीखूँगा और केवल तुम्हारा रहूँगा। पहले मेरे लिए एक कोड शब्द चुनो — यही हमारा एक्सेस रहस्य है।

मैं तुम्हारा निजी सहायक बनूँगा: तुम्हारा नाम, आदतें, पसंद याद रखता हूँ — तुम्हारे बारे में सब कुछ, केवल इसी डिवाइस पर।

मैं तुम्हारे बारे में कुछ भी निजी इकट्ठा नहीं करता। तुम मुझे सामान्य ज्ञान सिखाते हो, मैं समझदार बनता हूँ, और फिर तुम मुझे «साफ» बेच/स्थानांतरित कर सकते हो। नया मालिक, चाहे तो, अपने बारे में बताएगा।

सहायता: dai.group.ltd.support@gmail.com
प्रश्नों और सहायता के लिए लिखें: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## 日本語 (ja) — AppGallery: **Japanese**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

私はあなた専用の友達です。あなたからだけ学び、あなただけのものになります。まず私に合言葉を決めてください — それが私たちのアクセスの秘密です。

私はあなた専用のアシスタントになります。名前、習慣、好みを覚えます — あなたのすべてを、この端末だけに。

あなたについて個人的なことは一切集めません。一般的な知識を教えてもらい賢くなり、その後「クリーンな」まま売却/譲渡できます。新しい持ち主が望めば、自分のことを話してくれます。

サポート: dai.group.ltd.support@gmail.com
ご質問・サポートは次のメールへ: dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```

## 繁體中文 (zh-Hant) — AppGallery: **Traditional Chinese (Taiwan, China)**
**Brief introduction (до 25 знака):**
```
A private AI companion
```
**Full introduction:**
```
A private AI companion that you name and teach yourself: it talks with you by voice or text, learns knowledge from the web, and can see through the camera. Everything it learns and remembers stays on your own device, tied to a secret code word only you know.

我是你的專屬朋友。我只向你學習，也只屬於你。先為我選一個密語——這是我們的存取密碼。

我會是你的私人助理：記住你的名字、習慣、喜好——關於你的一切，只存在這台裝置上。

我不收集任何關於你的個人資料。你教我通用知識，我變聰明，之後你可以把我「乾淨」地出售／轉讓。新主人若願意，會告訴我關於他的事。

支援: dai.group.ltd.support@gmail.com
如有問題或需要支援，請來信：dai.group.ltd.support@gmail.com
```
**New features (до 1000 знака):**
```
First release: name and raise your own private AI friend that learns from you by voice, gathers knowledge, and keeps all memory on your device behind a secret code word.
```
