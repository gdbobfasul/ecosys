...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/gl1-candle-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/gl1-candle/hw-privacy.html` |
...
```
Launch the app; on first run pick one of 15 languages and continue past the intro and the policy screen. In the game a candle burns in the center and threats approach from the sides, above and the front, each marked on the rim. Raise the giant hand (left/right) against wind from a side, cover with the palm (up) against water from above, and lift the shield up front against blades and stones — hold the guard for storms. Defend the flame until each night's goal is met. No account, login, or internet connection is required. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/gl1-candle-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.gl1candle.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/gl1-candle/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
