...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/autoreply-bot-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/autoreply-bot/hw-privacy.html` |
...
```
No account or login is needed. On first launch choose a language, then tap 'Activate the robot — free' on the onboarding screen. Open the 'Demo' (inbox) tab and send a test message; the bot matches your rules and auto-replies, and the reply appears in the dashboard log. Nothing external is required to evaluate the app. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
