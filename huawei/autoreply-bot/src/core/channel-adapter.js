...
class LocalAdapter extends ChannelAdapter {
  constructor() { super('local', t('adp_demo_name')); }
  canSend() { return true; }
  status() {
    return { ready: true, note: t('adp_demo_note') };
  }
  async deliver({ text }) {
...
class NotificationChannelAdapter extends ChannelAdapter {
  ...
  status() {
    if (!isNativeReplyAvailable()) {
      return {
        ready: false,
        note: t('adp_need_native_access')
      };
    }
    return { ready: true, note: t('adp_native_ok_need_access') };
  }
