...
export async function kcyLogin(cfg) {
  ...
  if (!base) return { ok: false, reason: 'not-configured', note: tr('kcy_no_base') };
  const phone = String(cfg.phone || '').trim();
  const password = String(cfg.password || '').trim();
  if (!phone || !password) return { ok: false, reason: 'not-configured', note: tr('kcy_no_login') };
  ...
  if (data && data.needsRegistration) {
    return { ok: false, reason: 'no-account', note: tr('kcy_no_account') };
  }
  if (resp.status === 401) return { ok: false, reason: 'auth', note: tr('kcy_wrong_login') };
  return { ok: false, reason: 'http', note: tf('kcy_http', tr('kcy_login_word'), resp.status) };
}
...
export async function kcyCheck(cfg) {
  if (!kcyConfigured(cfg)) {
    return { ok: false, reason: 'not-configured', note: tr('kcy_need_config') };
  }
  kcyResetSession();
...
export async function kcyFetchFriends(cfg) {
  ...
  if (resp.status === 401) {
    kcyResetSession();
    return { ok: false, friends: [], reason: 'auth', note: tr('kcy_session_expired_pw') };
  }
  if (!resp.ok) return { ok: false, friends: [], reason: 'http', note: tf('kcy_http', '/api/friends', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, friends: [], reason: 'bad-json', note: tr('kcy_bad_friends') };
  }
...
export async function kcyFetchConversation(cfg, friendId) {
  ...
  if (resp.status === 401) { kcyResetSession(); return { ok: false, messages: [], reason: 'auth', note: tr('kcy_session_expired') }; }
  if (!resp.ok) return { ok: false, messages: [], reason: 'http', note: tf('kcy_http', '/api/messages', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, messages: [], reason: 'bad-json', note: tr('kcy_bad_messages') };
  }
...
export async function kcySend(cfg, { friendId, text }) {
  if (!friendId) return { ok: false, reason: 'no-target', note: tr('kcy_no_recipient') };
  const base = normBase(cfg.baseUrl);
  const auth = await ensureToken(cfg);
  ...
  if (resp.status === 401) { kcyResetSession(); return { ok: false, reason: 'auth', note: tr('kcy_session_expired') }; }
  if (resp.status === 403) {
    let d = {}; try { d = await resp.json(); } catch (_) { /* ignore */ }
    return { ok: false, reason: 'forbidden', note: (d && d.error) || tr('kcy_forbidden') };
  }
  if (!resp.ok) return { ok: false, reason: 'http', note: tf('kcy_http', tr('kcy_send_word'), resp.status) };
