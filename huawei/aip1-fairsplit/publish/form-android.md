...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-fairsplit-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-fairsplit/hw-privacy.html` |
...
```
No account, login, or internet connection is required. On first launch pick one of 15 languages and continue past the intro and the policy screen. Add a few people, then enter shared expenses (or switch mode to divide items or chores) and tap to compute the fair result; you can save and reload the case on-device. The AI helper is entirely OPTIONAL and off by default: it stays inactive unless you open Settings and enter your own AI key or proxy endpoint, and it then sends text only on your explicit action and only to the provider you chose. All core features work fully offline. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-fairsplit-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1fairsplit.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-fairsplit/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
