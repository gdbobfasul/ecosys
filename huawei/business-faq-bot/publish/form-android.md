# Android форма — business-faq-bot
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/business-faq-bot-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/business-faq-bot/hw-privacy.html` |
...
```
No account or login is needed. On first launch choose a language and tap 'Activate the bot'. Keep the sample knowledge base, then open the Demo chat and type a question containing a keyword; the bot replies from the FAQ, respects office hours, and offers escalation. All evaluation can be done offline in the demo chat. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```
