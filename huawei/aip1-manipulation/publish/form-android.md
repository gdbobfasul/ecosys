# Android форма — aip1-manipulation
...
| Поле | Попълни |
|---|---|
| App package | Manage packages → качи `apk/aip1-manipulation-huawei-release.apk` |
...
| Privacy policy URL | `https://selflearning.bot.nu/privacy/aip1-manipulation/hw-privacy.html` |
...
```
No account or sign-in is required and internet is optional. On first launch pick one of 15 languages and continue past the intro and the policy screen. Paste or type any message into the text box (or tap an example) and press Analyze to see the manipulation-risk breakdown; this works offline with built-in heuristics. To try the optional AI-assisted mode, open the gear/settings and enter your own AI proxy endpoint or Anthropic API key (stored only on the device); the analyzed text is sent to that provider only when you press Analyze. History is stored locally and can be cleared. Note: this build includes a 4-day trial; after 4 days it asks for a code word to continue — the code word is "кокошка" (kokoshka).
```

→ **Save** → **Submit**.

---
APK за качване: `apk/aip1-manipulation-huawei-release.apk` (release, подписан). Пакет: `com.pupikes.aip1manipulation.hw`.
Privacy е хостнат на `https://selflearning.bot.nu/privacy/aip1-manipulation/hw-privacy.html` (RuStore: `.../ru-privacy.html`) — публичен след деплой.
