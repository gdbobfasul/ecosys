...
router.post('/register', async (req, res, next) => {
  ...
  } catch (e) { debug.error('POST /register:', e && e.message); next(e); }
});
...
router.post('/login', async (req, res, next) => {
  ...
  } catch (e) { debug.error('POST /login:', e && e.message); next(e); }
});

// POST /api/wnb/logout
router.post('/logout', (req, res) => { debug.scoped(req, 'POST /logout')('старт'); return req.session.destroy(() => res.json({ ok: true })); });

// GET /api/wnb/me
router.get('/me', requireAuth, (req, res) => { debug.scoped(req, 'GET /me')('старт'); return res.json({ user: req.user }); });
