...
/**
 * PupikesGuardToken — ERC-20 с УНИКАЛНАТА защита „Vault Guard": таймлок на ГОЛЕМИТЕ трансфери ПЕР-ДЪРЖАТЕЛ
 * с възможност за ОТМЯНА. Идея (рядко срещана): всеки трансфер над личния праг на подателя се ЗАДЪРЖА за
 * период (напр. 1 час) като „чакащ"; през това време подателят ИЛИ неговият пазач (guardian) може да го
 * ОТМЕНИ. Така дори крадец да открадне ключа и да опита да източи, голямото прехвърляне се бави и се спира.
 * Малките трансфери (≤ прага) минават мигновено — ежедневието не страда.
 *
 * Всеки държател сам избира: праг (guardThreshold), забавяне (guardDelay) и пазач (guardian). Праг 0 = изкл.
 * Резервираните за чакащ трансфер токени се приспадат веднага от баланса (без двойно харчене), връщат се
 * при отмяна, кредитират се на получателя при изпълнение.
 */
contract PupikesGuardToken {
    // ── ERC-20 стандарт ──
    string public name;
    ...
    // ── Собственик (за метаданни/иницииращо мнтване; НЕ може да пипа чужди баланси) ──
