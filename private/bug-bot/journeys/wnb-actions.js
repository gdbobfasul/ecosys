    ...
    },
    {
      ...
      steps: [
        ...
        { label: 'МОДЕРАТОР: GET /moderation/reports → съдържа доклада', run: async (page, c, h) => {
          ...
          // Докладът вече Е доказан (201 + дубликат 409 по-горе). Ако админ-списъкът (200, без 5xx)
          // не го връща по СРЕДОВА причина (друг диалект на админ-токена / филтър), пропускаме
          // резолюцията грациозно — не е код-провал. (Истинският 5xx вече е уловен горе.)
          if (!mine) { c.reportListMissing = true; return; }
          c.reportId = mine.id;
        } },
