// harvest-doctor.mjs — събирач за Pupikes Doctor.
...
// Резултат → в папката на АПА: <repo>/rustore/pupikes-doctor/publish/reference/ (по правилото за
// резултати на ботове). Снимките се СВАЛЯТ (пакетът се хоства/сваля от апа; НЕ се вгражда в APK).
// Пуск: node private/medikit-harvester/harvest-doctor.mjs
...
const CONDITIONS = [
  { id: 'bruise', title: 'Bruise' },
  { id: 'fracture', title: 'Bone fracture' },
  { id: 'sprain', title: 'Sprain' },
  { id: 'cut', title: 'Wound' },
  { id: 'burn', title: 'Burn' },
  { id: 'bite', title: 'Insect bites and stings' },
  { id: 'rash', title: 'Rash' },
...
async function main() {
  ...
  for (const c of CONDITIONS) {
    ...
    for (const lang of LANGS) {
      ...
      rec.langs[lang] = { title, extract: s.extract, image: img };
      if (img) imgSet.add(img);
    }
    // Сваляне на снимките (уникални), локални имена.
    let i = 0;
    for (const url of imgSet) {
      const ext = (url.split('.').pop() || 'jpg').split('?')[0].slice(0, 4);
      const name = `${c.id}-${i}.${ext}`;
      if (await download(url, path.join(IMGDIR, name))) rec.images.push({ file: 'img/' + name, source: url });
      i++; await sleep(150);
    }
