...
router.get('/', async (req, res, next) => {
  try {
    const log = debug.scoped(req, 'GET /wanted');
    log('старт');
  ...
  } catch (e) { debug.error('GET /wanted:', e && e.message); next(e); }
});

// GET /api/fbp/wanted/:id — заявка + офертите ѝ (публично)
router.get('/:id(\\d+)', async (req, res, next) => {
  try {
    const log = debug.scoped(req, 'GET /wanted/:id');
    log('старт');
    const w = await one(
  ...
  } catch (e) { debug.error('GET /wanted/:id:', e && e.message); next(e); }
});

// POST /api/fbp/wanted  { title, description?, country, city? }
router.post('/', requireAuth, async (req, res, next) => {
  try {
    const log = debug.scoped(req, 'POST /wanted');
    log('старт');
    const p = req.body || {};
...
router.post('/:id(\\d+)/offer', requireAuth, async (req, res, next) => {
  ...
  } catch (e) { debug.error('POST /wanted/:id/offer:', e && e.message); next(e); }
});
