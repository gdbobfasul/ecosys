...
// Комбинираният файл NAME-SUGGESTIONS.md е за ВСИЧКИ апове (без едно конкретно) → docs/publish/name-checks/.
// Per-app предложенията отиват в huawei/<ап>/publish/ (по-долу). Правило на потребителя.
const OUT_DIR = path.join(__dirname, '..', '..', 'docs', 'publish', 'name-checks');
