...
```bash
# Проверка на едно или няколко имена → доклад в docs/publish/huawei/name-checks/<име>.md
...
- `actions` (по екран) → стъпки след таба: `{ "click": "css|text=…", "wait": ms }`, `{ "fill": "css", "value": "…" }` (попълва поле),
  `{ "scroll": "css", "offset": px }` (превърта елемента в горния край — резултати под сгъвката; offset = височина на фиксирана лента),
  `{ "click": "css", "files": ["samples/a.jpg", …] }` (кликът отваря избор на файл → подават се примерни файлове; пътища спрямо `publish/`),
  `{ "select": "css", "value": "…" }` (избира опция в `<select>`),
  `{ "css": "правила" }` (вкарва `<style>` само за снимката — напр. настолна двуколонна подредба на жив сайт → една колона).
- `url` (по екран) → адрес спрямо `remoteBase`/base (напр. `"profile.html"` при жив сайт с отделни страници) — пряко зареждане вместо кликове.
- `geolocation` → `{ "latitude": …, "longitude": … }` — разрешение + фалшиви координати за „търсене наблизо" (иначе „User denied Geolocation").
- При `remoteBase` сертификатните грешки се игнорират (снимаме съдържанието); живите API-та може да имат лимит на заявките (chat: 100/15 мин на IP → език по език с пауза).
- `storage` (общо и по екран) → допълнителни ключове в localStorage (обект → JSON) — семена с примерни данни.
- `i18n` → `{ "токен": { "bg": "…", "en": "…", … } }`; в `storage`/`state`/`fill.value` `{{токен}}` се заменя с текста за езика на снимката.
...
Пълната процедура по публикуване е в `docs/publish/huawei/how-to-publish.md`.
