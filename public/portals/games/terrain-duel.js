// AUTO-GENERATED от scripts/build-terrain-bg.js (източник: patch/DUEL_1280x960.html).
...
/* ============================================================
   ...
   ============================================================ */
function _mk(w,h){ if(typeof document!=='undefined'){const c=document.createElement('canvas');c.width=w;c.height=h;return c;} return require('@napi-rs/canvas').createCanvas(w,h); }
...
function aGiraffe(ctx,t,ph){const fur='#d8a84a',furL=lighten(fur,0.18),furD=darken(fur,0.32),sp='#9a6326';
  ...
  [[-7,lg],[-3,-lg],[6,-lg],[10,lg]].forEach(([lx,o])=>{ctx.beginPath();ctx.moveTo(lx,-24);ctx.lineTo(lx+o,0);ctx.stroke();});  // дълги крака
  ctx.fillStyle=fur;ctx.beginPath();ctx.ellipse(0,-28,13,7.5,0,0,6.283);ctx.fill();           // тяло
  ctx.save();ctx.translate(9,-32);ctx.rotate(-0.62+nk);                                         // шия
  ctx.fillStyle=furL;ctx.beginPath();ctx.ellipse(0,-17,4.5,19,0,0,6.283);ctx.fill();
  ctx.translate(0,-34);ctx.rotate(0.5);ctx.beginPath();ctx.ellipse(3,-1,6,4,0,0,6.283);ctx.fill();  // глава
  ...
  ctx.fillStyle=sp;[[-7,-29],[-2,-31],[3,-28],[-4,-26],[6,-30]].forEach(([sx,sy])=>{ctx.beginPath();ctx.ellipse(sx,sy,2.3,2,0,0,6.283);ctx.fill();}); // петна
}
function aTrex(ctx,t,ph){const sk='#5f6e3c',skL=lighten(sk,0.2),skD=darken(sk,0.32);
  ...
  ctx.strokeStyle=sk;ctx.lineWidth=7;ctx.lineCap='round';                                       // опашка
  ctx.beginPath();ctx.moveTo(-6,-16);ctx.quadraticCurveTo(-22,-13-st*3,-34,-3+st*4);ctx.stroke();
  ctx.strokeStyle=skD;ctx.lineWidth=5;                                                          // крака
  ctx.beginPath();ctx.moveTo(2,-16);ctx.lineTo(2+st*2,0);ctx.stroke();
  ctx.beginPath();ctx.moveTo(7,-16);ctx.lineTo(7-st*2,0);ctx.stroke();
  ctx.fillStyle=sk;ctx.beginPath();ctx.ellipse(2,-22,15,11,0,0,6.283);ctx.fill();               // тяло
  ctx.strokeStyle=skD;ctx.lineWidth=2.2;ctx.beginPath();ctx.moveTo(8,-22);ctx.lineTo(12,-17);ctx.stroke(); // ръчичка
  ctx.save();ctx.translate(13,-30);ctx.rotate(-0.2);                                            // шия/глава
  ctx.fillStyle=skL;ctx.beginPath();ctx.ellipse(0,-3,5,7,0,0,6.283);ctx.fill();
  ctx.translate(2,-9);ctx.beginPath();ctx.ellipse(5,0,10,6,0,0,6.283);ctx.fill();               // череп
  ctx.fillStyle=skD;ctx.beginPath();ctx.moveTo(7,3+jaw);ctx.lineTo(16,4+jaw);ctx.lineTo(7,6+jaw);ctx.fill(); // долна челюст
...
function buildScene1(W,H,seed){
  ...
  const animals=makeAnimalMgr(['rabbit','deer','lizard','boar','giraffe','trex'],3,1.0,place,rng,H);
  return {W,H,HOR,noise,bg,trees,bushes,tufts,rays,motes,dleaves,animals,place,rng,
          aTypes:['rabbit','deer','lizard','boar','giraffe','trex'],aScale:1.0};
}
...
function buildScene2(W,H,seed){
  ...
  // bushes filling side strips + a framing row just outside the path edge (on grass)
  for(let y=30;y<H;y+=lerp(95,150,rng())){
    bushes.push(genBush(rng,lE(y)-lerp(20,46,rng()),y,lerp(0.85,1.25,rng()),rng()<0.5));      // left frame (grass side)
    bushes.push(genBush(rng,rE(y)+lerp(20,46,rng()),y+lerp(-30,30,rng()),lerp(0.85,1.25,rng()),rng()<0.5)); // right frame
  }
  for(let y=10;y<H;y+=lerp(150,230,rng())){
    bushes.push(genBush(rng,lerp(15,PL-90,rng()),y+lerp(-40,40,rng()),lerp(0.7,1.1,rng()),rng()<0.5));
    bushes.push(genBush(rng,lerp(PR+90,W-15,rng()),y+lerp(-40,40,rng()),lerp(0.7,1.1,rng()),rng()<0.5));
  }
  ...
  const place=r=>{const y=lerp(80,H-80,r());return{x:lerp(lE(y)+40,rE(y)-40,r()),y};};
...
// ── стартер: пуска СЦЕНА 1 на подадения canvas ──
function startTerrainBg(canvas){
  var ctx = canvas.getContext('2d');
  var S = buildScene1(canvas.width, canvas.height, 20260606);
  var start = (typeof performance !== 'undefined' ? performance.now() : Date.now());
  (function loop(now){ renderScene1(ctx, S, now - start); requestAnimationFrame(loop); })(start);
  return S;
}
