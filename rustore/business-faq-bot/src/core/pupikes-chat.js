...
export async function pupikesLogin(cfg) {
  ...
  if (!base) return { ok: false, reason: 'not-configured', note: t('pupikes_missing_base') };
  const phone = String(cfg.phone || '').trim();
  const password = String(cfg.password || '').trim();
  if (!phone || !password) return { ok: false, reason: 'not-configured', note: t('pupikes_missing_login') };
  ...
  if (data && data.needsRegistration) {
    return { ok: false, reason: 'no-account', note: t('pupikes_no_account') };
  }
  if (resp.status === 401) return { ok: false, reason: 'auth', note: t('pupikes_wrong_login') };
  return { ok: false, reason: 'http', note: tf('pupikes_http', t('pupikes_http_login'), resp.status) };
}
...
export async function pupikesCheck(cfg) {
  if (!pupikesConfigured(cfg)) {
    return { ok: false, reason: 'not-configured', note: t('pupikes_not_configured') };
  }
  pupikesResetSession();
...
export async function pupikesFetchFriends(cfg) {
  ...
  if (resp.status === 401) {
    pupikesResetSession();
    return { ok: false, friends: [], reason: 'auth', note: t('pupikes_session_expired_pw') };
  }
  if (!resp.ok) return { ok: false, friends: [], reason: 'http', note: tf('pupikes_http', 'friends', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, friends: [], reason: 'bad-json', note: t('pupikes_bad_friends') };
  }
...
export async function pupikesFetchConversation(cfg, friendId) {
  ...
  if (resp.status === 401) { pupikesResetSession(); return { ok: false, messages: [], reason: 'auth', note: t('pupikes_session_expired') }; }
  if (!resp.ok) return { ok: false, messages: [], reason: 'http', note: tf('pupikes_http', 'messages', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, messages: [], reason: 'bad-json', note: t('pupikes_bad_messages') };
  }
...
export async function pupikesSend(cfg, { friendId, text }) {
  if (!friendId) return { ok: false, reason: 'no-target', note: t('pupikes_missing_recipient') };
  const base = normBase(cfg.baseUrl);
  const auth = await ensureToken(cfg);
  ...
  if (resp.status === 401) { pupikesResetSession(); return { ok: false, reason: 'auth', note: t('pupikes_session_expired') }; }
  if (resp.status === 403) {
    let d = {}; try { d = await resp.json(); } catch (_) { /* ignore */ }
    return { ok: false, reason: 'forbidden', note: (d && d.error) || t('pupikes_not_allowed') };
  }
  if (!resp.ok) return { ok: false, reason: 'http', note: tf('pupikes_http', t('pupikes_http_send'), resp.status) };
