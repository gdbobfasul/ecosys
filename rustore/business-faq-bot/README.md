# Бизнес FAQ робот — RUStore build
...
> Близнак: `huawei/business-faq-bot` (различава се само по app id, акцент и store документи).
...
```bash
npm run build            # → dist/
npm run gen:assets       # икона/сплаш PNG от акцента (#7b5cff)
...
src/
  ...
  store/rustore-sdk.js     # store-IAP STUB (no-op)
