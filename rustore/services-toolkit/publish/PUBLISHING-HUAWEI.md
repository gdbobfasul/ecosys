# Публикуване в Huawei AppGallery — Pupikes Toolkit
...
- **Huawei пакет:** `com.pupikes.servicestoolkit.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.servicestoolkit.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.servicestoolkit.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/services-toolkit/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit |
| bg | Български | **Bulgarian** | Pupikes Toolkit |
| ru | Русский | **Russian** | Pupikes Toolkit |
| uk | Українська | **Ukrainian** | Pupikes Toolkit |
| de | Deutsch | **German** | Pupikes Toolkit |
| fr | Français | **French (France)** | Pupikes Toolkit |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit |
| it | Italiano | **Italian** | Pupikes Toolkit |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit |
| ar | العربية | **Arabic** | Pupikes Toolkit |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit |
| ja | 日本語 | **Japanese** | Pupikes Toolkit |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/services-toolkit/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/services-toolkit/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
