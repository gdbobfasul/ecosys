# Публикуване в Huawei AppGallery — Pupikes Toolkit Pictures
...
- **Huawei пакет:** `com.pupikes.toolkit3drotate.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Pictures-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkit3drotate.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkit3drotate.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-3drotate/hw-privacy.html` |
...
| App name | `Pupikes Toolkit 3D Rotate` |
...
→ **OK** _(съдържателната категория „Photo & video" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit 3D Rotate |
| bg | Български | **Bulgarian** | Pupikes Toolkit 3D Rotate |
| ru | Русский | **Russian** | Pupikes Toolkit 3D Rotate |
| uk | Українська | **Ukrainian** | Pupikes Toolkit 3D Rotate |
| de | Deutsch | **German** | Pupikes Toolkit 3D Rotate |
| fr | Français | **French (France)** | Pupikes Toolkit 3D Rotate |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit 3D Rotate |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit 3D Rotate |
| it | Italiano | **Italian** | Pupikes Toolkit 3D Rotate |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit 3D Rotate |
| ar | العربية | **Arabic** | Pupikes Toolkit 3D Rotate |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit 3D Rotate |
| ja | 日本語 | **Japanese** | Pupikes Toolkit 3D Rotate |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit 3D Rotate |
...
|---|---|
| 1 (тип) | **Apps** |
| 2 (категория) | **Photo & video** |
| 3 (под-категория) | **AV editors** |

**Пълен път:** `Apps > Photo & video > AV editors`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-3drotate/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Pictures-huawei-release.apk` |
| Монетизация | Безплатно |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-3drotate/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
