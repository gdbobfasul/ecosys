...
enforceLicense('pupikes-toolkit-3drotate', 'rustore'); // лог на инсталация СЛЕД езика (rustore билд)
// Version: 1.0016
...
enforceLock();
mountEcosystem('pupikes-toolkit-3drotate'); // „Още от Pupikes" showcase
playIntro(); // кратко „Pupikes" интро при старт
startPromoAds('pupikes-toolkit-3drotate'); // реклами: старт (след интрото) + среда + край (PUPIKES_END_AD)
mountHelp('pupikes-toolkit-3drotate'); // универсален бутон „Помощ" (анонимен доклад → портал) // 4-дневно пробно заключване (виж core/lock.js)
mountPrivacyLink('pupikes-toolkit-3drotate'); // footer линк към политиката (Huawei 7.1) + заявка за изтриване на акаунт
mountLegalGate('pupikes-toolkit-3drotate'); // ЕКРАН 3: задължителни политики/предупреждения + отметка (стандарт)
  ...
  app.innerHTML = `
    <div class="view">
      <div class="hero">
        ...
        <h1>Pupikes Toolkit 3D Rotate</h1>
        <p>${esc(t('home_sub'))}</p>
      </div>
