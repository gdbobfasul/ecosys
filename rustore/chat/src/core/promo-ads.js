...
// после по СРЕДАТА (таймер) и в КРАЯ (игрите викат window.PUPIKES_END_AD() при game over).
//
// Каталогът се тегли ПЪРВО ОТ СЪРВЪРА (public/promo/pupikes-promo.json) — така се управлява
...
async function showAd() {
  try { if (window.__PUPIKES_INTRO_OFF__) return; } catch (e) {}          // без реклами при правене на снимки
  if (document.getElementById('pupikes-ad-ov')) return;
  ...
  const ov = document.createElement('div'); ov.id = 'pupikes-ad-ov';
  ...
  ov.innerHTML =
    '<div style="max-width:360px;width:100%;background:#111a2b;border:1px solid #24314a;border-radius:16px;overflow:hidden;position:relative">' +
      '<button id="pupikes-ad-x" style="position:absolute;top:8px;right:8px;z-index:2;background:rgba(0,0,0,.55);color:#fff;border:none;border-radius:16px;width:32px;height:32px;font-size:16px;cursor:pointer">✕</button>' +
      ...
      '<div style="padding:14px">' +
        ...
        '<button id="pupikes-ad-open" style="margin-top:12px;width:100%;padding:12px;border:none;border-radius:10px;background:#2ea043;color:#fff;font-weight:700;font-size:15px;cursor:pointer">Open ›</button>' +
        '<div style="text-align:center;margin-top:10px;font-size:11px;opacity:.55;color:#cdd9e5">Pupikes publisher 2026</div>' +
      '</div>' +
    '</div>';
  document.body.appendChild(ov);
  const close = () => { try { ov.remove(); } catch (e) {} };
  document.getElementById('pupikes-ad-x').onclick = close;
  document.getElementById('pupikes-ad-open').onclick = () => { openLink(a.storeUrl); close(); };
...
export function startPromoAds(selfId) {
  ...
  try { window.PUPIKES_END_AD = () => showAd(); } catch (e) {}           // край (game over)
}
