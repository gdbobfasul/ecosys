# Публикуване в Huawei AppGallery — Pupikes Toolkit Passwords
...
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Passwords-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkitpasswords.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitpasswords.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-passwords/hw-privacy.html` |
...
| App name | `Pupikes Toolkit Passwords` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Passwords |
| bg | Български | **Bulgarian** | Pupikes Toolkit Passwords |
| ru | Русский | **Russian** | Pupikes Toolkit Passwords |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Passwords |
| de | Deutsch | **German** | Pupikes Toolkit Passwords |
| fr | Français | **French (France)** | Pupikes Toolkit Passwords |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Passwords |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Passwords |
| it | Italiano | **Italian** | Pupikes Toolkit Passwords |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Passwords |
| ar | العربية | **Arabic** | Pupikes Toolkit Passwords |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Passwords |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Passwords |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Passwords |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-passwords/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Passwords-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-passwords/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
