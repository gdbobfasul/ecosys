...
enforceLicense('pupikes-toolkit-passwords', 'rustore'); // лог на инсталация СЛЕД езика (rustore билд)
// Version: 1.0016
...
enforceLock();
mountEcosystem('pupikes-toolkit-passwords'); // „Още от Pupikes" showcase
playIntro(); // кратко „Pupikes" интро при старт
startPromoAds('pupikes-toolkit-passwords'); // реклами: старт (след интрото) + среда + край (PUPIKES_END_AD)
mountHelp('pupikes-toolkit-passwords'); // универсален бутон „Помощ" (анонимен доклад → портал) // 4-дневно пробно заключване (виж core/lock.js)
mountPrivacyLink('pupikes-toolkit-passwords'); // footer линк към политиката (Huawei 7.1) + заявка за изтриване на акаунт
mountLegalGate('pupikes-toolkit-passwords'); // ЕКРАН 3: задължителни политики/предупреждения + отметка (стандарт)
  ...
  app.innerHTML = `
    <div class="view">
      <div class="hero">
        ...
        <h1>Pupikes Toolkit Passwords</h1>
        <p>${esc(t('home_sub'))}</p>
      </div>
