# Публикуване в Huawei AppGallery — Pupikes Ring Clash
...
- **Huawei пакет:** `com.pupikes.duel.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Ring-Clash-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.duel.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.duel.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/duel/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „Action Games" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Ring Clash |
| bg | Български | **Bulgarian** | Pupikes Ring Clash |
| ru | Русский | **Russian** | Pupikes Ring Clash |
| uk | Українська | **Ukrainian** | Pupikes Ring Clash |
| de | Deutsch | **German** | Pupikes Ring Clash |
| fr | Français | **French (France)** | Pupikes Ring Clash |
| es | Español | **Spanish (Spain)** | Pupikes Ring Clash |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Ring Clash |
| it | Italiano | **Italian** | Pupikes Ring Clash |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Ring Clash |
| ar | العربية | **Arabic** | Pupikes Ring Clash |
| hi | हिन्दी | **Hindi** | Pupikes Ring Clash |
| ja | 日本語 | **Japanese** | Pupikes Ring Clash |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Ring Clash |
...
|---|---|
| 1 (тип) | **Games** |
| 2 (категория) | **Action Games** |
| 3 (под-категория) | **Fighting** |

**Пълен път:** `Games > Action Games > Fighting`  _(игра — top ниво Games)_
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/duel/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Ring-Clash-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/duel/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
