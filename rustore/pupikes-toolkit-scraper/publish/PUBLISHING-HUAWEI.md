# Публикуване в Huawei AppGallery — Pupikes Toolkit Scraper
...
- **Huawei пакет:** `com.pupikes.toolkitscraper.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Scraper-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkitscraper.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitscraper.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-scraper/hw-privacy.html` |
...
| App name | `Pupikes Toolkit Scraper` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Scraper |
| bg | Български | **Bulgarian** | Pupikes Toolkit Scraper |
| ru | Русский | **Russian** | Pupikes Toolkit Scraper |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Scraper |
| de | Deutsch | **German** | Pupikes Toolkit Scraper |
| fr | Français | **French (France)** | Pupikes Toolkit Scraper |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Scraper |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Scraper |
| it | Italiano | **Italian** | Pupikes Toolkit Scraper |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Scraper |
| ar | العربية | **Arabic** | Pupikes Toolkit Scraper |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Scraper |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Scraper |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Scraper |
...
| 1 (тип) | **Apps** |
| 2 (категория) | **Tools** |
| 3 (под-категория) | **Tools** |

**Пълен път:** `Apps > Tools > Tools`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-scraper/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Scraper-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-scraper/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
