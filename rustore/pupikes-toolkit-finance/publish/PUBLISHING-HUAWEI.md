# Публикуване в Huawei AppGallery — Pupikes Toolkit Finance
...
- **Huawei пакет:** `com.pupikes.toolkitfinance.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Finance-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkitfinance.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitfinance.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/hw-privacy.html` |
...
| App name | `Pupikes Toolkit Finance` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Finance |
| bg | Български | **Bulgarian** | Pupikes Toolkit Finance |
| ru | Русский | **Russian** | Pupikes Toolkit Finance |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Finance |
| de | Deutsch | **German** | Pupikes Toolkit Finance |
| fr | Français | **French (France)** | Pupikes Toolkit Finance |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Finance |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Finance |
| it | Italiano | **Italian** | Pupikes Toolkit Finance |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Finance |
| ar | العربية | **Arabic** | Pupikes Toolkit Finance |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Finance |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Finance |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Finance |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Finance-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-finance/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
