...
// 11.09.2026 (v1.0029): примерните наблюдатели (sample:true) носят етикет „пример"; ctx.params.tab отваря под-таб от таблото.
...
export function siteLabel(k) { return tr(k); }   // за примерните данни (core/samples.js): found/notfound/online
...
export function renderSites(ctx) {
  injectSiteStyles();
  if (ctx && ctx.params && ctx.params.tab && TABS.includes(ctx.params.tab)) active = ctx.params.tab;   // от таблото „Общ преглед"
