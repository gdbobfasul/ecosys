...
// 11.09.2026 (v1.0029): RELAY РЕЗЕРВ (Huawei — тестерите са в Китай): ако прякото теглене пропадне или
// върне празно, опитваме през https://pupikes.app/api/relay/get?url=<encoded> (образец newslator/core/net.js).
// Relay-ът пуска само хостове от своя списък (RSS-и, certspotter, github, wikipedia…) — за останалите просто
// връща грешка и резултатът остава какъвто е бил от прякото теглене.

const RELAY_BASE = 'https://pupikes.app/api/relay/get?url=';
function relayUrl(url) { return RELAY_BASE + encodeURIComponent(url); }
function isOurs(url) { return /^https?:\/\/([^/]*\.)?pupikes\.app\//i.test(String(url || '')); }
...
export async function fetchText(url) {
  let s = '';
  try { s = await fetchTextDirect(url); } catch (_) { s = ''; }
  if (s || isOurs(url)) return s;
  try { return await fetchTextDirect(relayUrl(url)); } catch (_) { return ''; }   // relay резерв
}
...
export async function checkUptime(url) {
  const r = await checkUptimeDirect(url);
  if (r.online || isOurs(url)) return r;
  // relay резерв: ако сайтът е достъпен от нашия сървър → онлайн (сайтът работи, само пътят от телефона е затворен)
  try { const v = await checkUptimeDirect(relayUrl(url)); if (v.online) return { ok: true, online: true, status: v.status, viaRelay: true }; } catch (_) {}
  return r;
}
async function checkUptimeDirect(url) {
  const ch = CH();
  try {
    if (ch && ch.get) {
