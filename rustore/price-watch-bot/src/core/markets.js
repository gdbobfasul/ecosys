// Version: 1.0027
// 1.0027 (Huawei 3.1, 11.09.2026): всеки четец връща null при непълни данни (напр. празен отговор/{}),
// за да НЕ подменя вградения снимков пакет (core/snapshot.js) с празни редове; Binance spot свещи имат и
// огледалото data-api.binance.vision (публичните данни на Binance, отваря се и там, където api.binance.com не).
...
// Relay резерв (Huawei 3.1 — Китай: Yahoo/Binance блокирани): пряко → при null през нашия сървър.
const RELAY_BASE = 'https://pupikes.app/api/relay/get?url=';
export async function getJson(url, timeoutMs) {
  const d = await getJsonDirect(url, timeoutMs);
  if (d != null || /pupikes\.app/.test(url)) return d;
  return await getJsonDirect(RELAY_BASE + encodeURIComponent(url), Math.max(timeoutMs || 9000, 15000));
}
async function getJsonDirect(url, timeoutMs) {
...
export async function fetchFearGreed() {
  ...
  if (!it || !isFinite(parseInt(it.value, 10))) return null;
...
export async function fetchRSIFor(pair, interval, limit) {
  const q = '/api/v3/klines?symbol=' + pair + '&interval=' + interval + '&limit=' + (limit || 200);
  let rows = await getJsonCached('https://api.binance.com' + q, 3 * 60000);
  if (!Array.isArray(rows) || !rows.length) rows = await getJsonCached('https://data-api.binance.vision' + q, 3 * 60000);
...
export async function fetchFutures(sym) {
  ...
  if (!isFinite(mark)) return null;   // непълен отговор — оставяме вградения пакет
