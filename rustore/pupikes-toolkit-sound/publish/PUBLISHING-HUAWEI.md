# Публикуване в Huawei AppGallery — Pupikes Toolkit Sound
...
- **Huawei пакет:** `com.pupikes.toolkitsound.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Sound-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.toolkitsound.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.toolkitsound.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/pupikes-toolkit-sound/hw-privacy.html` |
...
| App name | `Pupikes Toolkit Sound` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Sound |
| bg | Български | **Bulgarian** | Pupikes Toolkit Sound |
| ru | Русский | **Russian** | Pupikes Toolkit Sound |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Sound |
| de | Deutsch | **German** | Pupikes Toolkit Sound |
| fr | Français | **French (France)** | Pupikes Toolkit Sound |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Sound |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Sound |
| it | Italiano | **Italian** | Pupikes Toolkit Sound |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Sound |
| ar | العربية | **Arabic** | Pupikes Toolkit Sound |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Sound |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Sound |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Sound |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/pupikes-toolkit-sound/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Sound-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/pupikes-toolkit-sound/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
