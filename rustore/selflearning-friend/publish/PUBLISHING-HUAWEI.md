# Публикуване в Huawei AppGallery — Pupikes Learning Buddy
...
- **Huawei пакет:** `com.pupikes.selflearningfriend.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Learning-Buddy-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.selflearningfriend.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.selflearningfriend.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/selflearning-friend/hw-privacy.html` |
...
→ **OK** _(съдържателната категория „Education" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes LB |
| bg | Български | **Bulgarian** | Pupikes LB |
| ru | Русский | **Russian** | Pupikes LB |
| uk | Українська | **Ukrainian** | Pupikes LB |
| de | Deutsch | **German** | Pupikes LB |
| fr | Français | **French (France)** | Pupikes LB |
| es | Español | **Spanish (Spain)** | Pupikes LB |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes LB |
| it | Italiano | **Italian** | Pupikes LB |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes LB |
| ar | العربية | **Arabic** | Pupikes LB |
| hi | हिन्दी | **Hindi** | Pupikes LB |
| ja | 日本語 | **Japanese** | Pupikes LB |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes LB |
...
|---|---|
| 1 (тип) | **Apps** |
| 2 (категория) | **Education** |
| 3 (под-категория) | **Learning** |

**Пълен път:** `Apps > Education > Learning`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/selflearning-friend/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
- **Data collection декларация** — по `app-profile.json`:
- **Микрофон (RECORD_AUDIO)** — виж app-profile.json
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Learning-Buddy-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/selflearning-friend/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
