# Публикуване в Huawei AppGallery — Pupikes Auto Answer
...
- **Huawei пакет:** `com.pupikes.autoreplybot.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Auto-Answer-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.autoreplybot.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.autoreplybot.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/autoreply-bot/hw-privacy.html` |
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Auto Answer |
| bg | Български | **Bulgarian** | Pupikes Auto Answer |
| ru | Русский | **Russian** | Pupikes Auto Answer |
| uk | Українська | **Ukrainian** | Pupikes Auto Answer |
| de | Deutsch | **German** | Pupikes Auto Answer |
| fr | Français | **French (France)** | Pupikes Auto Answer |
| es | Español | **Spanish (Spain)** | Pupikes Auto Answer |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Auto Answer |
| it | Italiano | **Italian** | Pupikes Auto Answer |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Auto Answer |
| ar | العربية | **Arabic** | Pupikes Auto Answer |
| hi | हिन्दी | **Hindi** | Pupikes Auto Answer |
| ja | 日本語 | **Japanese** | Pupikes Auto Answer |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Auto Answer |
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/autoreply-bot/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Auto-Answer-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/autoreply-bot/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
