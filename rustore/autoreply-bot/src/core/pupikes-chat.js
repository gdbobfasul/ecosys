// Version: 1.0001
// pupikes-chat.js — реална HTTP връзка към НАШИЯ чат (Pupikes chat app).
//
...
export function pupikesConfigured(cfg) {
...
export async function pupikesLogin(cfg) {
  const base = normBase(cfg.baseUrl);
  if (!base) return { ok: false, reason: 'not-configured', note: tr('pupikes_no_base') };
  const phone = String(cfg.phone || '').trim();
  const password = String(cfg.password || '').trim();
  if (!phone || !password) return { ok: false, reason: 'not-configured', note: tr('pupikes_no_login') };
  ...
  if (data && data.needsRegistration) {
    return { ok: false, reason: 'no-account', note: tr('pupikes_no_account') };
  }
  if (resp.status === 401) return { ok: false, reason: 'auth', note: tr('pupikes_wrong_login') };
  return { ok: false, reason: 'http', note: tf('pupikes_http', tr('pupikes_login_word'), resp.status) };
}
...
async function ensureToken(cfg) {
  ...
  const login = await pupikesLogin(cfg);
...
export function pupikesResetSession() {
  _session = { token: '', userId: '', baseUrl: '' };
}

// Лек тест на връзката: логва (ако трябва) и дърпа списъка с приятели веднъж.
export async function pupikesCheck(cfg) {
  if (!pupikesConfigured(cfg)) {
    return { ok: false, reason: 'not-configured', note: tr('pupikes_need_config') };
  }
  pupikesResetSession();
...
export async function pupikesFetchFriends(cfg) {
  const base = normBase(cfg.baseUrl);
  const auth = await ensureToken(cfg);
  ...
  if (resp.status === 401) {
    pupikesResetSession();
    return { ok: false, friends: [], reason: 'auth', note: tr('pupikes_session_expired_pw') };
  }
  if (!resp.ok) return { ok: false, friends: [], reason: 'http', note: tf('pupikes_http', '/api/friends', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, friends: [], reason: 'bad-json', note: tr('pupikes_bad_friends') };
  }
...
export async function pupikesFetchConversation(cfg, friendId) {
  const base = normBase(cfg.baseUrl);
  const auth = await ensureToken(cfg);
  ...
  if (resp.status === 401) { pupikesResetSession(); return { ok: false, messages: [], reason: 'auth', note: tr('pupikes_session_expired') }; }
  if (!resp.ok) return { ok: false, messages: [], reason: 'http', note: tf('pupikes_http', '/api/messages', resp.status) };

  let data;
  try { data = await resp.json(); } catch (_) {
    return { ok: false, messages: [], reason: 'bad-json', note: tr('pupikes_bad_messages') };
  }
...
export async function pupikesSend(cfg, { friendId, text }) {
  if (!friendId) return { ok: false, reason: 'no-target', note: tr('pupikes_no_recipient') };
  const base = normBase(cfg.baseUrl);
  const auth = await ensureToken(cfg);
  ...
  if (resp.status === 401) { pupikesResetSession(); return { ok: false, reason: 'auth', note: tr('pupikes_session_expired') }; }
  if (resp.status === 403) {
    let d = {}; try { d = await resp.json(); } catch (_) { /* ignore */ }
    return { ok: false, reason: 'forbidden', note: (d && d.error) || tr('pupikes_forbidden') };
  }
  if (!resp.ok) return { ok: false, reason: 'http', note: tf('pupikes_http', tr('pupikes_send_word'), resp.status) };
