# Публикуване в Huawei AppGallery — Pupikes Toolkit Authenticator
...
- **Huawei пакет:** `com.pupikes.authenticator.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Pupikes-Toolkit-Authenticator-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.authenticator.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.authenticator.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/authenticator/hw-privacy.html` |
...
| App name | `Pupikes Toolkit Authenticator` |
...
→ **OK** _(съдържателната категория „Tools" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Pupikes Toolkit Authenticator |
| bg | Български | **Bulgarian** | Pupikes Toolkit Authenticator |
| ru | Русский | **Russian** | Pupikes Toolkit Authenticator |
| uk | Українська | **Ukrainian** | Pupikes Toolkit Authenticator |
| de | Deutsch | **German** | Pupikes Toolkit Authenticator |
| fr | Français | **French (France)** | Pupikes Toolkit Authenticator |
| es | Español | **Spanish (Spain)** | Pupikes Toolkit Authenticator |
| es-MX | Español (MX) | **Spanish (Latin America)** | Pupikes Toolkit Authenticator |
| it | Italiano | **Italian** | Pupikes Toolkit Authenticator |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Pupikes Toolkit Authenticator |
| ar | العربية | **Arabic** | Pupikes Toolkit Authenticator |
| hi | हिन्दी | **Hindi** | Pupikes Toolkit Authenticator |
| ja | 日本語 | **Japanese** | Pupikes Toolkit Authenticator |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Pupikes Toolkit Authenticator |
...
| 1 (тип) | **Apps** |
| 2 (категория) | **Tools** |
| 3 (под-категория) | **Security** |

**Пълен път:** `Apps > Tools > Security`
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/authenticator/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
- **Data collection декларация** — по `app-profile.json`:
- **Камера (CAMERA)** — по функцията на приложението
- Мрежа: не.
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Pupikes-Toolkit-Authenticator-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/authenticator/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
