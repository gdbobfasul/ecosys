...
// Импорт от Aegis: избира файла през НАДЕЖДНИЯ picker (нативен на телефон → чете реалното
// съдържание; input в браузър), после плейн/криптиран поток с парола + дебъг при неуспех.
export async function importAegisFile(after) {
  const dbg = [];
  try {
    const picked = await pickTextFile();
    if (!picked) { return; }                                  // отказ
    const text = picked.text || '';
    dbg.push('file=' + (picked.name || '?') + ' size=' + (picked.size != null ? picked.size : '?') + ' chars=' + text.length);
    if (!text) { debugReport(dbg, { ok: false, reason: 'empty_file', detail: 'файлът се прочете празен — избери го от друго място (напр. Downloads) или експортирай пак от Aegis' }); return; }
