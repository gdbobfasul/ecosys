...
export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, H * 0.17, tf('level_n', this.levelNum), {
      fontFamily: 'system-ui, sans-serif', fontSize: '18px', color: '#fff'
    }).setOrigin(0.5);

    this.add.text(W / 2, H * 0.21,
      tf('score_line', this.score, this.dodges), {
  ...
  showNameEntry() {
    ...
    this.createNameInput(lastName() || t('default_name'));
  ...
  doSave() {
    ...
    this.saved = true;
    const name = (this.getNameValue() || lastName() || t('default_name')).slice(0, 24).trim() || t('default_name');
    setLastName(name);
