# Публикуване в Huawei AppGallery — Rustam picks cucumbers
...
- **Huawei пакет:** `com.pupikes.rustam.hw`
- **Билд (APK/AAB):** `apk/huawei/release/Rustam-picks-cucumbers-huawei-release.apk` (подписан; строи се от меню „билд/качване").
...
> Пакетът е `com.pupikes.rustam.hw`. AppGallery **не позволява** смяна на пакета на съществуващ запис — ако качиш
> в стар запис с друг пакет, отказва с „_the name of the uploaded package is different…_". Затова:
> 1. **My apps → New app** и задай **Package name = `com.pupikes.rustam.hw`** (фиксира се веднъж — точно това).
...
| **Политика за поверителност** | подава се като URL в AGC + показва се в апа (правило 7.1) | [`hw-privacy.html`](./hw-privacy.html) → `https://selflearning.bot.nu/privacy/rustam/hw-privacy.html` |
...
| App name | `Rustam picks cucumbers` |
...
→ **OK** _(съдържателната категория „Puzzle Games" се задава по-късно — на екрана Categorization/App information, не тук)_
...
|---|---|---|---|
| en | English | English (UK) _(по подр.)_ + **English (US)** | Rustam picks cucumbers |
| bg | Български | **Bulgarian** | Rustam picks cucumbers |
| ru | Русский | **Russian** | Rustam picks cucumbers |
| uk | Українська | **Ukrainian** | Rustam picks cucumbers |
| de | Deutsch | **German** | Rustam picks cucumbers |
| fr | Français | **French (France)** | Rustam picks cucumbers |
| es | Español | **Spanish (Spain)** | Rustam picks cucumbers |
| es-MX | Español (MX) | **Spanish (Latin America)** | Rustam picks cucumbers |
| it | Italiano | **Italian** | Rustam picks cucumbers |
| pt | Português | **Portuguese (Portugal)** + **Portuguese (Brazil)** | Rustam picks cucumbers |
| ar | العربية | **Arabic** | Rustam picks cucumbers |
| hi | हिन्दी | **Hindi** | Rustam picks cucumbers |
| ja | 日本語 | **Japanese** | Rustam picks cucumbers |
| zh-Hant | 繁體中文 | **Traditional Chinese (Taiwan, China)** + **Traditional Chinese (Hong Kong, China)** | Rustam picks cucumbers |
...
|---|---|
| 1 (тип) | **Games** |
| 2 (категория) | **Puzzle Games** |
| 3 (под-категория) | **Casual Games** |

**Пълен път:** `Games > Puzzle Games > Casual Games`  _(игра — top ниво Games)_
...
- **Privacy policy URL:** `https://selflearning.bot.nu/privacy/rustam/hw-privacy.html` (подава се като **адрес**, НЕ се качва HTML). Същата политика се отваря и ВЪТРЕ в апа (footer + екран за съгласие).
...
| Поле | Стойност |
|---|---|
| App package | Manage packages → качи `apk/huawei/release/Rustam-picks-cucumbers-huawei-release.apk` |
...
_Политика онлайн (Huawei): `https://selflearning.bot.nu/privacy/rustam/hw-privacy.html` — качва се на сървъра при билд/деплой (задължителния етап)._
