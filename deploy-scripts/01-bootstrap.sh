...
if [ -n "$1" ]; then
    ...
    if [ "$CHANGE_TARGETS" != "y" ] && [ "$CHANGE_TARGETS" != "Y" ]; then
        ...
        echo "  2) prod   — production VPS (${MAIN_DOMAIN})"
        ...
        case "$TARGET_PICK" in
            ...
            2) SERVER="${MAIN_DOMAIN}"; USER="root"; PORT="2222" ;;
...
# Главният домейн от единната конфигурация (resolveва TARGET_prod_SERVER навсякъде):
[ -f private/configs/domains.conf ] && . private/configs/domains.conf
...
EOF
            ...
            if [ "$DETECTED_TARGET" = "prod" ]; then
                # prod сървърът = главния домейн (от domains.conf) — НЕ го "печем" literal тук.
                sed -i \
                    -e "s|^TARGET_prod_USER=.*|TARGET_prod_USER=\"deploy\"|" \
                    -e "s|^TARGET_prod_PORT=.*|TARGET_prod_PORT=\"${NEW_SSH_PORT}\"|" \
                    "$TARGETS_FILE"
            else
                sed -i \
