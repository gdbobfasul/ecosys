...
#
# WhereNoBiz е САМОСТОЯТЕЛНО приложение (бъдещ Google Play / iOS).
# Този скрипт настройва САМО неговата услуга и маршрутизация:
#   1) systemd услуга kcy-wnb → node бекенд на WNB_PORT (по подразбиране 3011)
#   2) nginx подпапка в ОТДЕЛЕН include файл /etc/nginx/kcy-apps/wherenobiz.conf:
#        /wherenobiz/      → статичен frontend от WEB_ROOT
#        /api/wnb/         → node :PORT
#        /uploads/posts/   → node :PORT (качените картинки)
#
# ⚠ ИЗОЛАЦИЯ: създава САМО kcy-wnb.service и wherenobiz.conf.
#   НЕ докосва chat / eco3 / portals / основния сайт. Базата НЕ се пипа тук
#   (тя е отделно — опция 42 / 17-setup-wherenobiz-database.sh).
#
...
#   sudo ./19-setup-wherenobiz-service.sh --status   # само статус, без промени
##############################################################################
...
show_status() {
  ...
  if command -v curl &>/dev/null; then
    ...
    echo -e "  Health: $(curl -s -m 4 http://127.0.0.1:${P}/api/wnb/health 2>/dev/null || echo 'няма отговор')"
  fi
  echo -e "  nginx include: $([ -f "$NGINX_INC" ] && echo "$NGINX_INC ✓" || echo "липсва")"
  echo -e "  Активен в nginx: $(nginx -T 2>/dev/null | grep -q 'kcy-apps/wherenobiz' && echo да || echo "НЕ (пусни опция 2 веднъж)")"
}
...
[Unit]
Description=KCY WhereNoBiz Backend (самостоятелно приложение)
...
# KCY WhereNoBiz — самостоятелно приложение (НЕ е част от видимия сайт).
# Управлява се от 19-setup-wherenobiz-service.sh.
...
if nginx -T 2>/dev/null | grep -q 'kcy-apps/wherenobiz'; then
...
echo ""
echo -e "${YELLOW}══════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Готово: ${SVC} (WhereNoBiz).${NC}"
echo -e "${CYAN}  Тест:  https://<домейн>/wherenobiz/   и   /api/wnb/health${NC}"
