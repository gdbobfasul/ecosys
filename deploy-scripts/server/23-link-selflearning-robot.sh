...
if [ -z "$CONN_KEY" ]; then
  ...
  # Публикуваме тайния файл в ДВА уеб-корена, за да го намери апът и в двата случая:
  #   • /var/www/html               → ГЛАВНИЯТ nginx блок (server_name = IP/ts.net, root = тук).
  #       ТОВА ползва виртуалната машина БЕЗ публичен домейн: tailscale serve → nginx:8080 →
  #       главния блок, затова адресът /<ключ>/connection.bot.token сочи ИМЕННО ТУК.
  #   • /var/www/html/selflearning  → ОТДЕЛНИЯТ блок на домейна selflearning.bot.nu (root = тук),
  #       ползва се на production с публичен домейн.
  # Така /<ключ>/connection.bot.token се сервира независимо КОЙ блок поеме заявката — това
  # оправя „не се свързва" на виртуалната машина (преди файлът беше само в .../selflearning →
  # главния блок връщаше 404).
  PUB_ROOTS="/var/www/html"
  [ "$WEBROOT" != "/var/www/html" ] && PUB_ROOTS="$PUB_ROOTS $WEBROOT"

  # Съдържанието се сглобява ВЕДНЪЖ (един и същ времеви печат за двете копия).
  CONN_JSON=$(cat <<JSON
{
  "kind": "slf-connection",
  "version": 1,
...
JSON
  ...
  WROTE_ANY=0
  for _root in $PUB_ROOTS; do
    if [ ! -d "$_root" ]; then
      echo -e "  ${YELLOW}! Уеб-коренът $_root още го няма (пусни опция 33) — пропускам него.${NC}"
      continue
    fi
    _dir="$_root/$CONN_KEY"
    _path="$_dir/connection.bot.token"
    mkdir -p "$_dir" 2>/dev/null
    printf '%s\n' "$CONN_JSON" > "$_path" 2>/dev/null
    chmod 644 "$_path" 2>/dev/null
    if [ -f "$_path" ]; then
      WROTE_ANY=1
      # Запомни в регистъра + насрочи САМОУНИЩОЖЕНИЕ след 10 мин (детачнат таймер — НЕ блокира менюто).
      echo "$_dir" >> "$CONN_REGISTRY"
      SCHED=""
      if command -v systemd-run >/dev/null 2>&1; then
        systemd-run --quiet --collect --on-active="${EXPIRE_SECS}" \
          /bin/bash -c "rm -rf '$_dir'" >/dev/null 2>&1 && SCHED="systemd"
      fi
      [ -z "$SCHED" ] && setsid bash -c "sleep ${EXPIRE_SECS}; rm -rf '$_dir'" </dev/null >/dev/null 2>&1 &
    else
      echo -e "  ${RED}✗ Не успях да запиша $_path (права?).${NC}"
    fi
  done
  ...
  if [ "$WROTE_ANY" = 1 ]; then
    ...
    echo -e "  ${GRAY}Ръчно по-рано:  sudo rm -rf '/var/www/html/${CONN_KEY}' '/var/www/html/selflearning/${CONN_KEY}'   ·   owner-token-ът ОСТАВА.${NC}"
    # САМОПРОВЕРКА: наистина ли адресът се отваря отвън? Хваща точно тази грешка (грешен корен /
    # липсващ nginx на 8080 / изключен сертификат) ВЕДНАГА, вместо после в апа да гадаем.
    if command -v curl >/dev/null 2>&1; then
      _self_url="https://${SELF_DOMAIN}/${CONN_KEY}/connection.bot.token"
      if curl -fsS --max-time 8 "$_self_url" 2>/dev/null | grep -q 'slf-connection'; then
        echo -e "  ${GREEN}✓ Проверих: ${_self_url} се отваря — апът ще го намери.${NC}"
      else
        echo -e "  ${YELLOW}! Внимание: ${_self_url} още НЕ се отваря отвън — апът няма да се свърже така.${NC}"
        echo -e "  ${GRAY}  На виртуална машина без домейн: 1) пусни failover VM-подготовка (nginx да слуша на 8080),${NC}"
        echo -e "  ${GRAY}  2) включи Settings → Features → HTTPS Certificates в Tailscale конзолата, после пусни 81 пак.${NC}"
      fi
    fi
  else
    echo -e "  ${RED}✗ Не записах тайния файл в нито един уеб-корен (права/липсващи папки?).${NC}"
  fi
fi
echo -e "${YELLOW}══════════════════════════════════════════════════════════════════${NC}"
