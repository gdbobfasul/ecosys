...
if ! id "deploy" &>/dev/null; then
    ...
    echo "       Първо пусни опция 1 (Bootstrap нов сървър) за да го създаде"
    exit 1
fi
...
echo ""
echo "  инсталацията (от менюто: опция 2/4) — пуска се БЕЗ парола"
echo "  sudo systemctl restart kcy-chat"
echo "  sudo systemctl status nginx"
echo ""
echo "Тест:"
echo "  su - deploy"
echo "  пусни инсталацията от менюто: опция 2/4"
echo ""
