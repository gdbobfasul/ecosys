...
#
#   • server-install.sh / setup-wizard.sh / 01-setup-database.sh /
#     02-setup-domain.sh — install и setup скриптове
...
#
# Usage: sudo bash kcy-admin-sudo.sh
#
# ⚠ SECURITY WARNING ⚠
# Тъй като /var/www/deploy/deploy-scripts/ е owned от 'deploy' (за да може
# deploy.sh да качва нови версии), теоретично deploy може да замени
# server-install.sh с малициозно съдържание и да го run-не със sudo →
# root escalation.
#
...
if ! id "deploy" &>/dev/null; then
    ...
    echo "       Първо пусни server-prepare.sh за да го създаде"
    exit 1
fi
...
echo ""
echo "  sudo bash /var/www/deploy/deploy-scripts/server/server-install.sh"
