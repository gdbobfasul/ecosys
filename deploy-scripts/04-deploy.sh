...
# ═══ STEP 0: ПРАВНА ДОКУМЕНТАЦИЯ (Huawei/RuStore) → сървър ═══
# Архивът по-долу ИЗКЛЮЧВА папките huawei/ и rustore/ (огромни), а правните документи на
# приложенията живеят точно там (huawei/<ап>/publish/*-privacy.html и т.н.). Затова тук, ПРЕДИ
# архива, ги качваме право в /var/www/html/privacy/<ап>/ от локалната машина (където папките ги
# има) — за да работят правните линкове ВЪТРЕ в приложенията (изискване на Huawei 7.1/7.2 и
# RuStore). Автоматично, без питане. Не проваля деплоя при липса на връзка (сам се проверява).
if [ -f "deploy-scripts/sync-legal-pages.sh" ]; then
    log ""
    log "${GREEN}[0/5] Качване на задължителната документация (Поверителност/Условия/…)...${NC}"
    bash deploy-scripts/sync-legal-pages.sh 2>&1 | while IFS= read -r ln; do log "$ln"; done || true
fi
...
log ""
if [ -n "${KCY_IN_FULL_INSTALL:-}" ]; then
    log "  ${CYAN}(проверки на документи/сървизи — ВЕДНЪЖ накрая на пълната инсталация, не пер-машина)${NC}"
else
log "${YELLOW}═══════════════════════════════════════════════════${NC}"
...
fi   # край на гарда KCY_IN_FULL_INSTALL — при пълна инсталация проверките са ВЕДНЪЖ накрая (02)
