...
read -p "  Drop Databases — трия HLB/WNB базите и създавам от 0? [y/N, Enter = НЕ изтрива]: " A_DROP

WITH_ASSETS=0; case "${A_ASSETS,,}" in y|yes|да|д) WITH_ASSETS=1;; esac
RESET="";       case "${A_DROP,,}"   in y|yes|да|д) RESET=" --reset";; esac

echo ""
echo -e "  ${YELLOW}Асети: $([ "$WITH_ASSETS" = 1 ] && echo 'ДА (качват се с кода)' || echo 'не')   ·   Drop бази: $([ -n "$RESET" ] && echo 'ДА (създава от 0)' || echo 'НЕ (запазва данните)')${NC}"
echo ""
read -p "  Продължавам? [Y/n, Enter = Да]: " GO
case "${GO,,}" in n|no|не) echo "  Отказано."; exit 1;; esac
...
# ══ 1/4  DEPLOY (код+.env [+асети], npm авто, chat/eco3/portals) ══
step "1/4  Deploy на кода + .env + npm install + chat/eco3/portals"
if ! KCY_AUTO_NPM=1 KCY_WITH_ASSETS=$WITH_ASSETS DEPLOY_NO_PAUSE=1 \
