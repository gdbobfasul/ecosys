...
menu_title() {
    # ВАЖНО: item() подава номерата С водещ интервал за подравняване (" 2") — затова
    # сравняваме БЕЗ интервали и от двете страни, иначе заглавието „не се намира" и
    # банерът изписваше безсмисленото „ТОЧКА 2 — точка 2".
    local n="${1:-$CURRENT_OPT}" e num rest
    n="${n// /}"
    for e in "${MENU_INDEX[@]}"; do
        num="${e%%|*}"; num="${num// /}"
        if [ "$num" = "$n" ]; then rest="${e#*|}"; printf '%s' "${rest%%|*}"; return; fi
    done
    printf 'точка %s' "$n"
}
...
print_start_banner() {
    ...
    echo -e "${PURPLE}${RULE}${NC}"
    echo ""
    # Изнасяме точката и заглавието ѝ към ДЕТСКИТЕ скриптове (02/04/sync…): техните
    # завършващи надписи (lib/banner.sh) изписват ТОЧКАТА от менюто и сървъра,
    # НИКОГА името на скрипта-файл или услуга.
    export KCY_MENU_OPT="${opt// /}"
    export KCY_MENU_TITLE="$(menu_title "$opt")"
}
