...
for (const app of apps) {
  ...
  if (dh.network) {
    hw.push(line('ℹ️ ИНФО', 'Privacy Tags (7.2) — конзолна стъпка', 'Апът ползва мрежа (вкл. общия Pupikes слой: анонимен help доклад + промо каталог). Потоците са описани в политиката; при подаване ги декларирай и в Privacy Tags/описанието (за да съвпадат). Не е дефект в кода.'));
  } else {
  ...
  if (isIAP) {
    ...
    hw.push(line('✅ ОК', 'Реклами', 'Изскачащите промо реклами са автоматично ИЗКЛ. в платените апове (promo-ads е гейтнат по модела: показва се само при model=free). Остава единствено ненатрапчивото затваряемо балонче „✨ Pupikes" + доброволен showcase (ecosystem.js) — минава правилата.'));
  } else {
  ...
  // HTTPS
  if (dh.network) hw.push(line('✅ ОК', 'HTTPS (7.10)', 'Мрежовите заявки (вкл. Pupikes слоя) са по HTTPS. Провери, че няма http:// повиквания.'));
  // Китай
  ...
  // общ слой
  hw.push(line('ℹ️ ИНФО', 'Общ Pupikes слой', 'Всеки ап има help доклад (анонимен) + промо реклами (самореклама на други Pupikes апове). Няма трекинг SDK. Това вече е описано в новата политика — дръж я подадена.'));
  ...
  if (isIAP) {
    ...
    ru.push(line('✅ ОК', 'Реклами', 'Изскачащите промо реклами са автоматично ИЗКЛ. в платените апове (гейтнати по модела). Остава само ненатрапчивото затваряемо балонче „✨ Pupikes" + доброволен showcase — минава правилата.'));
  } else {
  ...
  ru.push(line('ℹ️ ИНФО', 'Общ Pupikes слой', 'Help доклад (анонимен) + промо реклами. Декларирано в политиката; RuStore иска и деклариране на типовете събирани данни в конзолата.'));
