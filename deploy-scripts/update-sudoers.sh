...
# инсталираният път (там, където Deploy слага кода за постоянно)
REMOTE_SCRIPT="/var/www/kcy-ecosystem/deploy-scripts/server/03-kcy-admin-sudo.sh"
...
ssh -t -o ConnectTimeout=15 -p ${PORT} "${USER}@${SERVER}" "sudo ${REMOTE_SCRIPT}"
RC=$?
echo ""
if [ $RC -eq 0 ]; then
    ...
    echo -e "${YELLOW}  Ако 'command not found' → първо пусни 2) Deploy (за да качи 03 на сървъра).${NC}"
    echo -e "${YELLOW}  Ако иска парола → 03 още не е в стария whitelist; въведи sudo паролата на ${USER} веднъж.${NC}"
fi
