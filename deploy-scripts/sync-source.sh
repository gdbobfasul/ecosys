...
if [ $RC -eq 0 ]; then
    ...
    echo -e "${YELLOW}  тези промени НЕ са влезли. Пусни „2) Deploy проекта → prod\" (носи .env),${NC}"
    echo -e "${YELLOW}  после пак тази опция за рестарт. Само код-промени → нищо повече не трябва.${NC}"
else
