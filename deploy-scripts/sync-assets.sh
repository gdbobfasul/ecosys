...
REMOTE_SCRIPT="${PROJECT_DIR}/deploy-scripts/server/15-sync-assets.sh"
...
# ── активиране ──
# 15-sync-assets.sh вече е инсталиран на сървъра и е whitelist-нат. Подаваме архива.
...
if [ $RC -eq 0 ]; then
    ...
    echo -e "${YELLOW}  при ПЪРВО ползване. Прехвърлянето на асети още не е${NC}"
    echo -e "${YELLOW}  разрешено без парола на сървъра. Направи това ВЕДНЪЖ:${NC}"
    echo -e "${YELLOW}═══════════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${CYAN}1)${NC} Пусни ${GREEN}опция 4 (Deploy проекта) → prod${NC}"
    echo -e "     Това качва новите deploy-scripts/ на сървъра"
    echo -e "     (вкл. сорс, асети и обновените sudo права)."
    echo ""
    echo -e "  ${CYAN}2)${NC} После пусни ${GREEN}опция 30 (Update sudoers на сървъра)${NC}."
    echo ""
    echo -e "  ${CYAN}3)${NC} След това пусни пак ${GREEN}опция 6 (Прехвърли само АСЕТИ)${NC} — вече без парола."
    echo ""
fi
