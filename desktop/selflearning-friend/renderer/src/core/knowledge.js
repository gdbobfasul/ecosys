...
// Десктоп клонинг (Electron)? Засичаме по userAgent или по флага от preload.
// Това копие пази паметта си на КОМПЮТЪРА, отделно от телефона.
function isDesktopClone() {
  try {
    if (typeof window !== 'undefined' && window.SLF_DESKTOP === true) return true;
...
export function storageLocationLabel() {
  ...
  try {
    if (typeof window !== 'undefined' && window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Preferences) {
      ...
      // Десктоп клонинг (Electron): хранилището е localStorage на ТОЗИ компютър,
      // напълно отделно от телефона. Това копие може да получи различни задачи.
      place = 'localStorage (локално на компютъра)';
    }
  } catch (_) { /* пада към браузър */ }

  const where = desktop ? 'на този компютър' : 'на това устройство';
  const ep = sourcesSettings().serverEndpoint;
  return {
    place,
    desktop,
    serverConfigured: !!ep,
    text: '📍 Знанието се пази: ЛОКАЛНО ' + where + ' (' + place + ')'
