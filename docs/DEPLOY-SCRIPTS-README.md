...
### Стъпка 1: `deploy.sh` — качване (от Windows/Mac)
...
> **СЛЕДВАЩА СТЪПКА:** `sudo bash server-install.sh`

### Стъпка 2: `server-install.sh` — инсталация (на сървъра)
...
> **СЛЕДВАЩА СТЪПКА:** `sudo bash 01-setup-database.sh`

### Стъпка 3: `01-setup-database.sh` — база данни (на сървъра)

```bash
sudo bash 01-setup-database.sh
```
...
PG_PASSWORD=                       # само за postgresql (генерира се от 01-setup-database.sh)
...
# ── Security ──
JWT_SECRET=                        # генерира се от 01-setup-database.sh
SESSION_SECRET=                    # генерира се от 01-setup-database.sh
```

`01-setup-database.sh` **добавя** DB настройките без да трие останалото.
...
## Поведение на server-install.sh
...
| Файл | Къде | Какво |
|------|------|-------|
| `deploy.sh` | Твоята машина | Качва в staging |
| `deploy-scripts/windows/deploy.ps1` | Windows PowerShell | Същото |
| `deploy-scripts/windows/deploy.bat` | Windows CMD | Wrapper |
| `deploy-scripts/server/server-prepare.sh` | Сървър (sudo, еднократно) | Подготвя потребители и директории |
| `deploy-scripts/server/server-install.sh` | Сървър (sudo) | Копира, настройва, стартира |
| `deploy-scripts/server/01-setup-database.sh` | Сървър (sudo) | PostgreSQL / SQLite |
| `deploy-scripts/server/02-setup-domain.sh` | Сървър (sudo) | Nginx + SSL + systemd |
