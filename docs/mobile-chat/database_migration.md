...
- Run with: `./scripts/migrate-database.sh database/YOUR_MIGRATION.sql`
...
### Custom migration:
```bash
./scripts/migrate-database.sh database/db_migration_YOUR_FILE.sql
```

### Example:
```bash
./scripts/migrate-database.sh database/db_migration_EXAMPLE_future.sql
```
...
4. Run on production:
```bash
./scripts/migrate-database.sh database/db_migration_MY_NEW_FEATURE.sql
```
