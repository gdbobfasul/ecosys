...
```
Congratulations! You have successfully enabled HTTPS on https://${MAIN_DOMAIN}
```
...
```bash
sudo nano /etc/nginx/sites-available/${MAIN_DOMAIN}
```
...
```bash
sudo ln -s /etc/nginx/sites-available/${MAIN_DOMAIN} /etc/nginx/sites-enabled/
```
...
* Certbot автоматично ще:
  ...
  * издаде сертификат
  * добави `listen 443 ssl;`
  * настрои SSL сертификата (`/etc/letsencrypt/live/${MAIN_DOMAIN}/fullchain.pem`)
  * добави редирект от HTTP → HTTPS

---

💡 **Бонус съвет:** Ако искаш, веднага можеш да сложиш **self-signed сертификат** за HTTPS тест, докато изчакаш лимита на Let’s Encrypt. Това ще позволи браузърът да се отвори на [https://${MAIN_DOMAIN}](https://${MAIN_DOMAIN}) (с предупреждение), без да чакаш 27.01.2026.

Искаш ли да направим това временно решение с self-signed сертификат?



Да, вече виждам твоето `sites-enabled` — **конфигурацията е правилно активирана**, nginx ще отговаря за `${MAIN_DOMAIN}` на HTTP.
...
* Ще попита:
  ...
  * Country, State, City, Organization и Common Name (тук сложи: `${MAIN_DOMAIN}`)
* Това създава два файла:

  * `/etc/ssl/private/selfsigned.key` → private key
  * `/etc/ssl/certs/selfsigned.crt` → сертификат
...
ls -l /etc/nginx/sites-enabled/
total 0
lrwxrwxrwx 1 root root 46 Jan 27 09:30 ${MAIN_DOMAIN} -> /etc/nginx/sites-available/${MAIN_DOMAIN}
...
* Най-вероятно `default` блокът в `/etc/nginx/sites-available/default` слуша на порт 80 и **„краде“ HTTP заявките** за `${MAIN_DOMAIN}`.
